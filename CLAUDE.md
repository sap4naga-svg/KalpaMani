...
### The infrastructure-feasibility gap, and ADR-0019 — ACCEPTED, and the implementation gap
...
implementation does not yet conform to that architecture.**
...
structurally prevented from importing or invoking the assessment read surface.

#### The implementation gap — open, and stated plainly

**The architecture is accepted. The code is not corrected.** These are four different states and
...
**The current dormant acquisition implementation still uses the pre-ADR-0019 shared collision
path**, which issues a conditional HeadObject after a 412. **The current dormant implementation is
therefore not deployable under the authoritative architecture.** **No claim is made that the
current implementation already has zero acquisition HEAD operations**, and **no claim is made that
the ADR-0018-specific write-only publication surface already exists** — it does not.
**Infrastructure remains blocked until the production correction is separately authorized,
implemented, reviewed, merged and synchronized.**
...
created, and Run A, Run B and the combined assessment have never run.


### The legitimate duplicate-payload collision, and ADR-0020 — PROPOSED

[ADR-0020](docs/decisions/ADR-0020-request-scoped-qualification-payload-identity.md) proposes a
narrow amendment to one identity rule. **ADR-0020: PROPOSED / NOT IN FORCE.** **ADR-0020 carries
no authority until it is independently reviewed and merged**, and it is deliberately **not
registered as a merged ADR**.
...
incompatibility in the accepted architecture** visible.
...
6. **ADR-0020 is proposed to correct the qualification payload identity without weakening
   ADR-0019.**
7. **PR #48 cannot be reviewed or merged until ADR-0020 is independently reviewed, merged and
   synchronized, and PR #48 is corrected against it.**
...
published. Under the current derivation the second such write lands on an occupied name, ADR-0019
correctly fails it closed, and the run halts — so **the accepted complete-run shape is unreachable
whenever a legitimate duplicate payload occurs**. **This is an identity and key-contract problem,
not a reason to weaken write-only acquisition.**
...
receive different claim and record names. Only the payload key is under-scoped.

#### What ADR-0020 proposes
...
PR #48:                                   OPEN / UNMERGED / BLOCKED ON ARCHITECTURE
ADR-0020:                                 PROPOSED / NOT IN FORCE
ADR-0020 implementation:                  NOT AUTHORIZED
PR #48 correction against ADR-0020:       NOT BEGUN
infrastructure:                           BLOCKED
deployment:                               NOT PERFORMED
Run A:                                    NOT RUN
Run B:                                    NOT RUN
combined assessment:                      NOT RUN
```

**PR #48 is untouched by the ADR-0020 proposal** — not edited, rebased, amended, reviewed,
commented on, retitled, closed or merged, and auto-merge was not enabled on it. It was inspected
read-only, to confirm the conflict.
...
governing ones, and **ADR-0020 is proposed rather than in force**.
