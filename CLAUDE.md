...
| **One unmistakable flag** | `--run`, `--live`, `--execute` and `--force` are each **refused by name**, with a reason, so a wrong reflex fails loudly. The check is for exact `True`: a truthy string, a `1` or a lookalike authorizes nothing |
