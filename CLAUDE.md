...
| **ADR-0018 implementation · the licensed object-byte read surface · the `data/qualify/` package · the two new entry points · the two new IAM roles · Run A · Run B · the assessment run** | **NOT AUTHORIZED** — ADR-0018 is **ACCEPTED / IN FORCE**, and **the merge approved architecture only**. **ADR-0018 implementation: NOT AUTHORIZED · infrastructure mutation: NOT AUTHORIZED · Run A: NOT AUTHORIZED · Run B: NOT AUTHORIZED · assessment: NOT AUTHORIZED.** **Implementation, infrastructure mutation and execution stay three separate gates and are never collapsed into one.** An **offline implementation candidate exists on an open pull request; it is UNMERGED, NOT ACCEPTED and BLOCKED**, and **it cannot be merged until it is corrected against the clarified ADR** under a **separately authorized implementation correction** and an **independent re-review**. **The clarification amendment is PROPOSED and not effective until merged**, and it **authorizes none of the later gates** |
...
IN FORCE       ADR-0009 provider-specific code (PR #13 merged)
               ...
               ADR-0018 bounded private empirical Sharadar qualification --
                        ...
                        LISTING ANYWHERE. A CLARIFICATION AMENDMENT IS PROPOSED
                        AND IS NOT EFFECTIVE UNTIL MERGED -- IT RECORDS THE
                        1,800-SECOND ACQUISITION ELAPSED-TIME DEADLINE ON AN
                        INJECTED MONOTONIC CLOCK AND THE COMBINED ASSESSMENT,
                        SUPERSEDES THE CANONICAL ONE-LOCATOR ARITHMETIC OF 97
                        GETOBJECT AND 98-99 OPERATIONS, ADDS NO AUTHORIZATION
                        AND CORRECTS NO IMPLEMENTATION. THE OFFLINE
                        IMPLEMENTATION CANDIDATE IS UNMERGED, NOT ACCEPTED AND
                        BLOCKED PENDING AN ACCEPTED CLARIFICATION, A SEPARATELY
                        AUTHORIZED IMPLEMENTATION CORRECTION AND AN INDEPENDENT
                        RE-REVIEW. A SANITIZED RUNTIME-AREA LISTING INCIDENT IS
                        ...
                        ZERO, AND THE LICENSED OBJECT-BYTE READ SURFACE DOES NOT
                        EXIST IN THE ACCEPTED TREE. ADR-0018 IMPLEMENTATION NOT
                        AUTHORIZED,
...
**A clarification amendment is PROPOSED and is not effective until merged.** An independent
...
Run B. Both are written into ADR-0018 by a **documentation-and-governance-only** pull request, and
**that amendment carries no authority until it merges**. **ADR-0018 itself remains ACCEPTED / IN
FORCE as architecture only**, and the amendment **adds no authorization of any kind**.

**The offline implementation candidate remains an UNMERGED implementation candidate, and it is
BLOCKED.** It may not be merged until there is an **accepted clarification**, a **separately
authorized implementation correction** and an **independent re-review**. **Clarifying an
architecture is not correcting an implementation**, and the clarification pull request corrects
none: it changes no source file, no entry point and no test of that candidate.
...
clarification amendment:                  PROPOSED -- NOT EFFECTIVE UNTIL MERGED
offline implementation candidate:         UNMERGED AND BLOCKED -- pending accepted
                                          clarification, a separately authorized
                                          implementation correction, and an
                                          independent re-review
ADR-0018 implementation:                  NOT AUTHORIZED -- and not started
...
licensed object-byte read surface         DOES NOT EXIST in the accepted tree -- until
                                          separately authorized implementation is merged
