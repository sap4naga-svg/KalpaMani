...
merged as PR #88 — defined, and none delivered; Decision M's implementation is in an open pull
request, pending independent review**. **The full Cockpit V1 does not exist**: **C5 and C7 are both NOT
...
with `CandidateIntent` as its only output.** That work is implemented and is in this open pull
request, pending independent review. **It authorizes and establishes nothing operational**: no
...
```text
offline Brain foundation:                         IMPLEMENTED / SYNTHETIC ONLY / PENDING REVIEW
