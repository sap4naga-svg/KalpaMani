...
disclosure control carries one availability badge per distinct non-`AVAILABLE` state among its
section's settled widgets**, in the order the contract's vocabulary declares them — glyph and label,
never a number, never a skeleton, never a precedence the contract does not define — and **every
distinct provenance badge** its section carries. A pending read contributes no badge and hides none.
...
**The full content-to-location table, the state matrix and the ten test obligations are in
...
its primary panel settled and not the shell's freshness indicator — p50 ≤ 1 000 ms and no sample
above 1 500 ms; **PB2** first contentful paint p50 ≤ 800 ms and no sample above 1 200 ms,
...
**PB5** transferred bytes ≤ 750 KB compressed per route, the document included, at 1 KB = 1 000 bytes.
**Bounded query time is `DEFERRED`** until a read-model boundary exists to measure — **and while it is
deferred this row reads at most `PARTIAL`, however PB1–PB5 report**, because the row names three
targets and a named target without a definition is not satisfied.
...
missing sample; a first failure followed by a passing re-run is `PASS — ON RE-RUN` with both runs
reported, never a clean pass. The zero-is-invalid rule is a validity rule for these timings and byte
...
reproduce deterministically from a URL — scenario, mode and any declared variant, with `env`,
`period` and `gran` held at their defaults as a stated narrowing — with transient loading covered by
U7 at component level and route-level `ERROR` recorded **`NOT YET CONSTRUCTIBLE`** on every read-model
route until a deterministic selector exists, an open obligation that bars a complete reading and is
not satisfied by the eleven-state reference screen; the **fixed viewport list** is the three original
widths for every route and **all six** reference viewports for `/`; and a combination is
**inapplicable only under a cited rule** VC-R1 to VC-R6 — **an uncited inapplicability is a gap, and
a gap reads `PARTIAL`**, while a cited inapplicability is coverage and never bars completion. **A full Cartesian product is neither required nor silently waived**: the interpretation
is stated so that accepting it is a decision.

**The inventory is 439 nominal snapshots over 32 route identifiers**, nine of which exist. Capture
...
every image**. State is set up **from the URL only**, with one stated exception — the four mobile
expanded-state rows, which exist only once §12.1 is implemented and are expanded by keyboard because
expansion is deliberately not in the URL. Every baseline records the tree, OS, browser
...
— plus a structural pass over **every registered route and both deep destinations** (one `h1`, `main`
reachable, ordered headings, the context bar read), each traced to its accepted clause. **The mobile
journey is `BLOCKED` until §12.1 is implemented, never `NOT APPLICABLE`, and a blocked journey bars a
pass.** Value-bearing states announce their value with its qualification; every other state announces
no value. Headings and landmarks, distinct control
...
primary combination — the blocked mobile journey included — run by the named assessor with zero S1
