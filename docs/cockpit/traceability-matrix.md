...
|---|---|---|---|---|---|
| 1 | Executive Overview | **full** | every contributing subsystem; governance parts already real | **C4** | every tile renders its own availability and as-of; strategy capital and broker equity shown separately, never substituted; **initial planned risk, current open planned risk and permitted risk render as three separate facts, the second with its as-of**; five ten-second answers in the first viewport at 1440 × 900 |
...
| 3 | Positions & Exposure | **full** | portfolio runtime — **NOT IMPLEMENTED** | **C5** | borrow state comes from a record, never from price; groupings are displayed and never computed as a permitted exposure; **initial and current open planned risk render as separate columns and a moving stop changes only the second** |
| 4 | Strategy Performance | **full** | strategy runtime — **NOT IMPLEMENTED**; **G7 OPEN** | **C5** | Breakout and Pullback keep separate module attribution and share family context; no diversification or alpha claim without evidence; every result attributed to an exact strategy version; **`strategy.capacity` is admissible only through the §12.3.3 qualified-model interface — every *applicable* input present, a recorded *positive* model qualification, and its parameters, search domain, granularity and observed-execution baseline displayed with the value**; **no capacity is rendered from strategy capital, buying power, available cash, gross exposure or any limit, and no absent capacity is rendered as zero — while a computed zero renders `AVAILABLE` as the measurement it is**; **a still-feasible upper grid endpoint renders `PARTIAL` as a lower bound rather than a maximum, and an empty feasible set renders `NOT_APPLICABLE` and never zero**; **per-version capacities are never summed into a portfolio capacity** |
| 5 | Strategy Health | **full** | strategy runtime — **NOT IMPLEMENTED** | **C7** | only the seven ADR-0026 health states render; the view causes no transition; a degradation shows the research queue entry it created; **the rolling tail loss renders its window, its population, its tail fraction and its observation count beside the value**, is computed over **closed trades of one exact version** on a **closed-trade count window**, carries §12.1's sign so a loss is negative, renders a **positive or zero value as a measurement** and a **short population as `INSUFFICIENT_OBSERVATIONS` carrying no value — whether or not trades were also excluded, with the exclusion count still disclosed and `PARTIAL` reachable only once the minimum is met**; **no health-state transition is caused, created or implied by it** |
...
| 12 | Risk Dashboard | **full** | risk engine — **NOT IMPLEMENTED** | **C5** | strictly read-only; governed research values displayed unchanged and labelled as research parameters; **the four risk quantities are separately labelled, every permitted value shows its policy reference, and a missing assessment renders unavailable rather than zero or `NOT_APPLICABLE`** |
...
| 15 | Champion / Challenger | **full** | research runtime — **NOT IMPLEMENTED** | **C7** | readiness is displayed and never conferred; no promotion path exists from this view; **every cited result renders its evaluation class, and exploratory reuse is never displayed as fresh out-of-sample evidence** |
...
| 18 | Hypothesis Registry | **full** | learning engine — **NOT IMPLEMENTED** | **C7** | a registration is immutable; results render as linked appended records; the amendment chain is visible; failed and abandoned runs count against the budget; **the exposure ledger and the trial budget render across the research lineage, so a new registration identity resets neither** |
...
| 24 | Project / Qualification Governance | **full** | **already real** — tracked repository governance facts | **C4** | public-safe facts only; **real tracked facts render with `REPOSITORY_TRACKED` provenance and are never labelled `SYNTHETIC`**; **publishing the view externally requires a recorded release authorization in addition to the `PUBLIC_SAFE` label**; **P1–P9 render `UNEVALUATED`**; each gate read independently; Run B authorization and its date gate render as two separate facts |
...
| 33 | Cockpit Read-Model Architecture | **contracts only** | none — architecture | **C3** | every response carries the full envelope; an unknown `schema_version` is rejected, not rendered; **every catalogued read model has a field-level payload contract, every endpoint declares its response type, filters, sorts, page sizes, extent bound, cursor and closed error codes, and no `_ref` lacks a declared resolution** |
...
| 36 | Trade History & Trade Detail | **full** | portfolio and execution runtimes — **NOT IMPLEMENTED** | **C5** history and basic detail, **C6** full lifecycle | a fill is never counted as a trade; a partial exit reduces rather than closes; a missing event renders as a gap and never as an inference; **business status and data completeness render as separate columns**; **R multiple divides by initial planned risk and a moving stop never changes it**; the four concepts stay on separate screens; **the owner's manual activity is never adopted as platform evidence** |
