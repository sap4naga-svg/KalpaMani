...
| 4.3 | per build: the **build input v2** (ADR-0055 — build identity, ≤ 32 completed `RECEIPT_VERIFIED` production runs each bound by the SHA-256 of its admitted locator, ledger digest, ≤ 24 h, ≤ 8 KiB) | the launch tool cuts it from the ledger and the preserved locators beside it | **after the first acquisition** | `/kalpamani/production/research-build/input` | `inputs.py`; example `build-input.v2.synthetic.json` (v1 historical only) | **PRIVATE** — Parameter Store only |
