...
| D-17 | the disposition of the **six invalid-target R-6 subcells** (`*-RUN-OTHER-REVISION`, `*-RUN-OTHER-FAMILY`, `*-RUN-OTHER-CLUSTER`; ADR-0047 §12, proposed): accept them as `BLOCKED` with the stated dependencies (R-6 and S8 stay incomplete), or authorize the governed decoys (a second own-family verification revision per actor, an empty decoy cluster — a Terraform change) and the other-actor probe family as the wrong-family target — each its own plan/apply, registration re-issue and re-run. **Neither choice makes R-6 `PASSED`**: an accepted deferral remains `BLOCKED`; only the six subcells' recorded refusals against existing targets can pass them; proceeding past a later gate while R-6 is `BLOCKED` is an explicitly accepted risk, labelled `RISK ACCEPTED — R-6 UNVERIFIED`, never verification | S8 |
