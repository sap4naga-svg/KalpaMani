...
| D-10 | the resolution of the **verification-only path** (readiness §3, §8) — an ADR decision, before R-1/R-2 | S9 |
...
| V-4 | the **build configuration** document | the build owner-inputs file (`build_configuration`) → image | the accepted `BuildConfiguration.document()` shape; **its `accepted_schemas` digests can be observed only from real deliveries, so the first build image follows the first completed acquisition** |
...
| V-11 | the **R-3 verification record** and its SHA-256 | the owner's evidence; `terraform.tfvars` → `production_r3_verification_digest` | supplied only for a record that says *verified* |
