...
| D-15 | per launch, the **written authorization record** (`kalpamani-launch-authorization/v1`: actor, kind, identity, ≤ 24 h) the launch tool requires beside its flag — one record per launch, never reused (example: `examples/production/launch-authorization.synthetic.json`) | S9, S10 |
...
| V-16 | per launch: the **launch-inputs record** (`kalpamani-launch-inputs/v1`: cluster, execution role, binding key, platform version; per actor the task role, subnet, security groups, and the registered production and verification revision ARNs with their image digests, configuration digests and code commits) | owner-private, transcribed from Terraform outputs and the generation records | the launch tool compiles `CompiledLaunch` from it and from nothing typed; **not exemplified** (it carries ARNs) — its field set is `launch_records.parse_launch_inputs` |
...
| V-18 | per launch: the **launch record** (`kalpamani-launch-record/v1`) the tool writes — task ARN, revision, digests, commit, identity, input digest, the acquisition slice — and the sanitized evidence document | owner-private, under the private root, beside the ledger | the record is what the receipt line is verified against (`--complete-row`); it is never exported and never enters a pull request |
...
| E-11 | R-2: the probe's observed result and attempt count from the receipt, **and separately** the isolation verdict — `FAILED` on `CONNECTED`, otherwise `INCONCLUSIVE` until the corroboration the verification ADR defines is recorded (readiness §3.3) | S9 (build) |
