...
**Tests added or rewritten:** `test_production_launch_store.py` (16, incl. eight competing threads
reserving once), `test_production_isolation_verdict.py` (44), `test_production_launch_script.py`
(rewritten; reservation, interruption after reservation and after `RunTask`, evidence and ledger write
failures, concurrent distinct identities, recovery refusal, each bound authorization category, each
equivalence substitution, each verdict reason), `test_production_launch_records.py` (specification,
task-definition evidence, bound equivalence).
