...
| 4.3 | per build: the **build input v1** (build identity, ≤ 32 completed `RECEIPT_VERIFIED` production rows, ledger digest, ≤ 24 h) | `/kalpamani/production/research-build/input` | example `build-input.v1.synthetic.json` | ADR-0040; `inputs.py` | 4.2 completed and its row `RECEIPT_VERIFIED` | **after the first acquisition** |
...
| 4.6 | **the bounded provider-qualification cycle for the pagination-v2 limit `L` and the payload/parser/memory ceilings** (ADR-0053 §3, §8; proposed, effective on that ADR's merge) — before the constants code cycle, the O-5 recompilation and the whole-run replacement of acquisition run 1 (historical, not buildable: every tickers/actions page full at 10,000 rows; 2,147 rows re-served across two tickers offsets); run 2's specification superseded and preserved; S10c blocked | the qualified limit `L` and the byte/row/memory ceilings → the constants code cycle → every recompiled plan digest | one qualification record per candidate (row counts, sizes, digests, parser time, peak memory, task resources, licence, safety margin) | owner-inputs D-18 / D-19; ADR-0053 | 4.1, 4.5 | **not started — no value is qualified** |
