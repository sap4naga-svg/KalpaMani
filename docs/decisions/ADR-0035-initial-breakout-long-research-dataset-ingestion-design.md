...
| provider time — **rule P-3 (evidence-derived bound, version-specific)** | only for a row version with **version-specific evidence** that *these bytes* existed at a known time — a dated archival capture containing the identical row, or a per-version vendor record. **The bound is taken from the evidence itself, never from a schedule**: (a) if the evidence carries an instant with an evidenced timezone, `provider_available_upper_bound` = that capture instant; (b) if the evidence carries only a date, the bound is the **conservative end of that date interval** — the last instant of the date in the evidenced timezone, or, with the timezone unknown, the last instant of that date anywhere on Earth (UTC−12:00). Both need a derivation the contract does not have — `ProviderBoundDerivation` offers only `FIRST_SEEN_UPPER_BOUND` and `DELIVERY_WINDOW` — so this route requires the extension **`ProviderBoundDerivation.VERSION_EVIDENCE_UPPER_BOUND`**, proposed here and **gated** until a contract ADR adds it; until then the row falls to P-2. (c) `ProviderBoundDerivation.DELIVERY_WINDOW` — the one expressible member — may be used **only when the evidence explicitly establishes that the matching version was available by a specific scheduled delivery** (e.g. a per-version vendor delivery record naming that delivery); the bound is then that delivery's instant. **A service-inception date and a delivery schedule alone establish that *some* version of a row was delivered on a date; they do not establish that the version retrieved today is that version**, because the vendor revises rows in place (`lastupdated`, `PSR-SHD-022`), and they never supply a bound on their own (criterion G2-D) | same, keyed to the action date, same evidence and expressibility requirements | not applicable |
...
**Rule order.** P-3 applies only to a row version with version-specific evidence, and only through a
derivation the contract can express — `DELIVERY_WINDOW` with explicit per-version delivery evidence now,
`VERSION_EVIDENCE_UPPER_BOUND` for capture-derived bounds once its extension exists; otherwise P-1 only
once its own extension exists and its three prerequisites are met for the dataset; otherwise P-2 —
**first-seen timing is the default, the only rule that needs no evidence beyond the acquisition record,
and the fall-back whenever a bound the evidence supports cannot be expressed by the accepted contract.** Every rule yields a **bound**; no rule in this design writes an exact `provider_available_time`.
...
| **T-3** capture-derived bound, instant with timezone | `v2` plus an archival capture whose row bytes are identical to `v2`, captured at 2019-03-06T18:05:00 America/New_York (evidenced) | with the `VERSION_EVIDENCE_UPPER_BOUND` extension present: `provider_available_upper_bound` = 2019-03-06T23:05:00Z, the capture instant itself — **not** the 17:30 or 23:30 delivery of that date; the manifest cites the evidence digest; `provider_available_time` stays null. Without the extension: P-2 (`FIRST_SEEN_UPPER_BOUND` = `b2`); a fixture that emits any capture-derived bound without the member **fails** |
| **T-4** capture after the last scheduled delivery | as T-3, but captured at 2019-03-06T23:47:00 America/New_York — after the vendor's 23:30 ET delivery | the bound is the capture instant, 2019-03-07T04:47:00Z; a fixture that substitutes the 23:30 delivery instant (2019-03-07T04:30:00Z) **fails** — the schedule never supplies a bound the evidence does not; the row's earliest admissible `as_of` is later than any delivery-based reading would give |
| **T-5** date-only capture, timezone unknown | as T-3, but the capture carries only the date 2019-03-06 and no timezone | the bound is the conservative end of the date interval: 2019-03-06T23:59:59.999 at UTC−12:00 = 2019-03-07T11:59:59.999Z; a fixture that uses an assumed timezone, the vendor's ET delivery, or the start of the date **fails**; without the extension the row is P-2 |
| **T-6** scheduled delivery with explicit per-version evidence | `v2` plus a per-version vendor delivery record stating that this version was included in the 2019-03-06 23:30 ET delivery | `DELIVERY_WINDOW` applies (expressible today): bound = 2019-03-07T04:30:00Z; a fixture that applies `DELIVERY_WINDOW` from the schedule alone, without the per-version record, **fails** (that is T-2) |
...
| **V-1** revision inheritance across rules | `v1` admitted under P-3 with 2019 capture evidence; `v2` arrives with no evidence | `v2` is P-2 only; the resolution map shows the per-version split; a fixture in which `v2` inherits `v1`'s P-3 bound **fails** |
...
| **G2-D** | **Evidence-derived bound (rule P-3) established per row version**: version-specific evidence that the retrieved bytes existed at a known time — dated archival captures (with instant and timezone, or date-only with the conservative interval end) or per-version vendor delivery records — for the rows it is applied to, with the bound taken from that evidence; a service-inception date and a delivery schedule are context and **never a bound**; capture-derived bounds also need the `VERSION_EVIDENCE_UPPER_BOUND` contract extension by its own ADR | the rule, its version-evidence requirement, the conservative date-interval arithmetic, the `DELIVERY_WINDOW`-only-with-explicit-evidence restriction and the fall-through to P-2 are implementable now; the extension is a contract ADR | **yes** — the archival captures or per-version records do not exist in this repository and would come from an additional source; without them P-3 admits nothing |
...
- **The repository gains a reviewable target for the production data plane** that reuses every accepted
  contract and proposes two `UniverseExclusionReason` members, two `ProviderBoundDerivation` members and one
...
- **Five gates follow this one, each its own authorization**: infrastructure design (principals,
  ...
  extension ADR for `VENDOR_DATE_UPPER_BOUND` and `VERSION_EVIDENCE_UPPER_BOUND` if P-1 and capture-derived
  P-3 are to exist at all); the first bounded ingestion
...
- **Snap an evidenced capture to the nearest scheduled delivery.** Rejected: a capture at 23:47 proves
  availability by 23:47, not by 23:30; the bound is the evidence's own instant, or the conservative end of
  its date interval when only a date is evidenced, and a schedule may stand in only when a per-version
  record explicitly names the delivery.
- **Express an evidence-derived bound through a member that means something else.** Rejected: writing a
  capture-derived bound as `DELIVERY_WINDOW` or `FIRST_SEEN_UPPER_BOUND` would record a derivation the
  data did not have; until the contract can say what happened, the row is first-seen.
...
vocabulary additions (UNRESOLVED_CORPORATE_ACTION,
    ATTRIBUTE_UNAVAILABLE, VENDOR_DATE_UPPER_BOUND,
    VERSION_EVIDENCE_UPPER_BOUND):                PROPOSED — implemented only under later gates
...
## 9. Amendment (2026-09-18) — acquisition-plan shape under pagination v2; run 1 replaced whole (ADR-0053)
...
**Superseded rule (historical):** §3.1's plan table — `tickers` "one full snapshot, paginated" at 10,000 and fixed page counts per window. **Rule under ADR-0053:** every group (the tickers snapshot; each canonical actions window; each stocks session) is one data request at the qualified limit `L` plus one completion probe at offset `L`; windows are unchanged unless ADR-0053 §2.13's partition fallback applies to a dataset. **§3.5 as implemented — each run's `stocks` and `actions` rows are mapped through the `tickers` snapshot acquired in the same run — is confirmed, not changed**, and it is why acquisition run 1 (whose snapshot is truncated and offset-unstable) is replaced as a whole under a new identity rather than patched: stocks are not copied onto the old run. The expected replacement shape is ≈ 94 requests (88 stocks + 2 tickers + 4 actions; write ceiling `1 + 3×94 + 1 = 284` if the count is 94), final only after qualification and recompilation of every affected plan digest (the O-5 program included). *[Since accepted: ADR-0053 is ACCEPTED / IN FORCE on the merge of PR #129 — 2026-09-18T02:01:09Z, merge commit `3e8c9cb5c13141c0353d4bb62c3e65c498badf1b`, ordered parents `7cfb9bac…` then `cef2db91…`, merge tree identical to the reviewed head tree — and this amendment with it, as governance and contract only; nothing implemented, qualified or deployed by that merge; true when written, not rewritten.]* *[Amended 2026-09-18 by ADR-0053 §11 (proposed in the pull request introducing that section; effective only on its merge): the tickers snapshot is acquired per required Sharadar `table` as its own logical group — for the first observation build exactly `table=stocks`, derived from the accepted consumers (§11.4) — with `permaticker` the identity within the group and tables never combined; a data page below `L` needs no probe (§11.2) — true when written, not rewritten.]* *[Amended 2026-09-18 by ADR-0053 §12 (proposed in the pull request introducing that section; effective only on its merge): for the pagination-v2 Route-A actions input the row identity is the schema-bound canonical full-row event identity (`sharadar-actions-event-identity/v1`: all seven governed typed fields in accepted order, exact duplicates refused, no precedence, no silent combining, ordering independent of delivery order) replacing the coarse `(security_id, date, action)` key of §3.6 as implemented; D-20's `table=stocks` tickers group may be reused under §12.5 — true when written, not rewritten.]*

*[Implemented (2026-09-18, the pagination-v2 offline code cycle authorized under ADR-0053 §13 — offline code and tests only; no image, registration or run): the plan contract is `kalpamani-production-acquisition-plan/v2` — one data coordinate per group at offset 0 and the governed limit, a conditional-probe authorization instead of a compiled second page, the worst-case provider-call ceiling `2N` held under the accepted 96 (so at most 48 groups per run) and the write ceiling `1 + 3N + 1`; the accepted program planner (`program.py`) recompiles O-5 from bounded runs and its derived totals, not the indicative ≈ 94/275 or 10/847/2,561 figures, are the recorded arithmetic.]*
