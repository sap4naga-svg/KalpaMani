...
## 7. Amendment (2026-09-18) — the acquisition entry parses the completion probe only (ADR-0053)
...
**Superseded rule (historical):** the acquisition entry composes the opaque processing path — nothing is parsed task-side (ADR-0036 §2.2, ADR-0043 §2). **Rule under ADR-0053 §2:** after a group's completion probe is published, the acquisition entry runs the accepted parser on the **probe response only** to prove zero rows and equal schema; any other result halts the run (`ACQUISITION_HALTED`, exit 22) after the probe's own three conditional writes, with the locator `PARTIAL` and never `COMPLETE`. **Data responses stay opaque** — never parsed, never inspected task-side; no read, listing or extra S3 operation is added; the receipt's counts are unchanged in kind. The factories, refusals and receipt line of this ADR are otherwise untouched. *[Since accepted: ADR-0053 is ACCEPTED / IN FORCE on the merge of PR #129 — 2026-09-18T02:01:09Z, merge commit `3e8c9cb5c13141c0353d4bb62c3e65c498badf1b`, ordered parents `7cfb9bac…` then `cef2db91…`, merge tree identical to the reviewed head tree — and this amendment with it, as governance and contract only; nothing implemented, qualified or deployed by that merge; true when written, not rewritten.]* *[Amended 2026-09-18 by ADR-0053 §11 (proposed in the pull request introducing that section; effective only on its merge): the acquisition entry's probe check applies to every probe that is issued — only after a data page of exactly `L` rows (§11.2) — true when written, not rewritten.]*

*[Implemented (2026-09-18, the pagination-v2 offline code cycle authorized under ADR-0053 §13 — offline code and tests only; no image, registration or run): under §11.2 and §13.3 the acquisition processing parses **every data response** with the accepted parser (bounded by the 32 MiB whole-body ceiling and the coordinate's governed row limit) to count rows and take the schema digest, issues the probe only for an exactly-full page, and halts **before the group's writes** on `DATA_PAGE_MALFORMED`, `PAGE_OVER_LIMIT`, `PROBE_DATA_BEARING`, `PROBE_REFUSED`, `PROBE_MALFORMED`, `PROBE_SCHEMA_MISMATCH` or `PROVIDER_CALLS_EXCEEDED`; the probe body is never written; the receipt's provider-request count includes probes actually issued; the compiled configuration of the acquisition entry carries the pinned `pagination_targets` (`kalpamani-pagination-v2-targets/v1`) and cannot run under other limits, ceilings or memory targets.]*
