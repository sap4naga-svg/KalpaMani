...
**Superseded rule (historical):** the manifest's pagination record is ADR-0042's (`sharadar-pagination-admission-v1`: admitted and empty group counts per dataset, `does_not_establish`). **Rule under ADR-0053:** policy version `sharadar-pagination-admission-v2`; per group the limit `L`, the data-page row count, the probe result (parsed, zero rows), schema equality, `rows_total == data_rows`, and the probe payload's digest recorded as delivery-completeness evidence; probe pages contribute no rows to any Silver or Gold artifact and no bytes to their digests. Everything else in §2 — content-addressed Silver and Gold, the name-addressed manifest published last, the derived `run_id`, the 3,600 s deadline — is unchanged. *[Since accepted: ADR-0053 is ACCEPTED / IN FORCE on the merge of PR #129 — 2026-09-18T02:01:09Z, merge commit `3e8c9cb5c13141c0353d4bb62c3e65c498badf1b`, ordered parents `7cfb9bac…` then `cef2db91…`, merge tree identical to the reviewed head tree — and this amendment with it, as governance and contract only; nothing implemented, qualified or deployed by that merge; true when written, not rewritten.]* *[Amended 2026-09-18 by ADR-0053 §11 (proposed in the pull request introducing that section; effective only on its merge): the manifest's v2 pagination record carries, per group, whether a probe was required and issued, and for tickers the `table` predicate (§11.2–§11.3) — true when written, not rewritten.]* *[Amended 2026-09-18 by ADR-0053 §12 (proposed in the pull request introducing that section; effective only on its merge): the Silver actions row key for the pagination-v2 Route-A input becomes `(security_id, event_identity)` per ADR-0053 §12.4, with `ACTIONS_DUPLICATE_EVENT` and `ACTIONS_IDENTITY_COLLISION` refusals and the manifest recording the identity-contract version — true when written, not rewritten.]*

*[Implemented (2026-09-18, the pagination-v2 offline code cycle authorized under ADR-0053 §13 — offline code and tests only; no image, registration or run): the manifest contract is `kalpamani-production-build-manifest/v2` — the pagination record carries `groups_probed` beside admitted and empty counts, every run block carries `probes_issued` and `provider_calls`, the `unresolved_contracts` block is replaced by `identity_contracts` naming `sharadar-actions-event-identity/v1`, and `actions_identity_version` is bound into the build `run_id` and the transformation record; per ADR-0053 §13.3 the probe is not a payload object — its response SHA-256, byte count, row count, schema digest and outcome are retained in the group's acquisition record (`kalpamani-production-acquisition-record/v2`) and locator entry (`kalpamani-production-run-locator/v2`).]*
