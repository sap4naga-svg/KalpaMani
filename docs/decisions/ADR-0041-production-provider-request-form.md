...
## 7. Amendment (2026-09-18) — one data request plus one completion probe per group; multi-page data refused (ADR-0053)
...
**Superseded rule (historical):** §3's pagination clause — compiled offset pages per window (`PAGES_PER_WINDOW` tickers 4 / actions 2 / stocks 2) transmitted as the vendor's `skip`, with the second page a completeness probe and a non-empty later page an "undocumented-ordering case". **Rule under ADR-0053 §2:** each group or window is acquired by **exactly one** governed data request at limit `L` (offset 0) and **exactly one** completion probe at offset `L` with the same table, filters, date bounds, ordering, projection and every other immutable parameter; `skip` still transmits the probe's offset and nothing else changes in the serialization; a plan describing more than one data-bearing request per group is refused. `L` is a qualified per-dataset constant (ADR-0053 §3); if no safe single-page `L` exists for a dataset, the fallback is a deterministic server-side partition by a stable predicate or key — **offset pagination does not return as the fallback**. The assumptions this ADR names (`from`/`to` inclusivity, no terminal-page signal, no stable order across offsets) are unchanged, and the third is now observed rather than assumed. *[Since accepted: ADR-0053 is ACCEPTED / IN FORCE on the merge of PR #129 — 2026-09-18T02:01:09Z, merge commit `3e8c9cb5c13141c0353d4bb62c3e65c498badf1b`, ordered parents `7cfb9bac…` then `cef2db91…`, merge tree identical to the reviewed head tree — and this amendment with it, as governance and contract only; nothing implemented, qualified or deployed by that merge; true when written, not rewritten.]* *[Amended 2026-09-18 by ADR-0053 §11 (proposed in the pull request introducing that section; effective only on its merge): the completion probe is issued only when the data response carries exactly `L` rows (§11.2); the production cross-section form admits the documented `table` parameter for the tickers dataset only, with a value from the accepted predicate set `{ "stocks" }` (§11.3–§11.4); the `PSR-SHD-134` row records the parameter's documented description — true when written, not rewritten.]*

*[Implemented (2026-09-18, the pagination-v2 offline code cycle authorized under ADR-0053 §13 — offline code and tests only; no image, registration or run): the compiled coordinate carries its `predicate` (the tickers `table=stocks` pair; none elsewhere), the probe is `ProductionRequest.probe()` — the same shape at offset `L` — issued only for an exactly-full page (§11.2), and the adapter refuses a missing, different or additional tickers predicate, a predicate on a windowed table, and a page over the dataset's governed limit, each before any transport invocation.]*
