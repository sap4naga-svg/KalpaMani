...
ADR-0036 runtime foundations (code):              OFFLINE / SYNTHETIC-ONLY / NO PROCESSING / NO IMAGE / NO ENTRY POINT — PR #95, open
production ingestion/backfill/update:             NOT AUTHORIZED / NOT RUN
...
actors and their launch tool, written under a separate authorization after PR #94 merged and proposed in
**PR #95 (open, unmerged)**:
...
| **inputs** | the acquisition and build input contracts of §2.6 — 8 KiB checked before parsing, 24 h validity, the compiled plan digest supplied by the caller (none exists yet), spent identities refused, at most 32 distinct run identities, the ledger digest over the rows as delivered; the input digest is over the **delivered bytes** |
...
| **runner** | the task-side order — environment, binding, input, self-check over documented metadata fields, identity proof, release barrier — and then the honest terminal state **`HALTED_PROCESSING_NOT_IMPLEMENTED`** |
...
**What does not exist, and is not claimed.** No acquisition processing (no secret retrieval, no provider
request, no Bronze publication, no locator publication), no build processing (no Silver, Gold or manifest
publication), no production image, no entry point, no compiled production plan, no spent-identity registry,
no R-3 tooling, no strategy logic and no cockpit change. The concrete SSM, ECS and EC2 adapters construct
the exact requests the ADR prescribes and classify failures structurally, and **have only ever been exercised
against synthetic fakes**: mocked results are not AWS verification, and nothing here has sent an AWS request.

```text
production runtime foundations (code):             OFFLINE / SYNTHETIC-ONLY — PR #95, open, unmerged
processing (acquisition, build):                   NOT IMPLEMENTED — the runner halts after the release barrier
production image / entry point / compiled plan:    NONE
