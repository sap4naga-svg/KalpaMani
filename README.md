...
ADR-0047 negative launches + permission subcells (code):         OFFLINE / SYNTHETIC-ONLY / FAKES ONLY / NEVER RUN — proposed in an open, unmerged pull request (ADR-0047 PROPOSED, NOT IN FORCE); ReleaseMode on the specification and the record, the launcher's WITHHELD and MISMATCHED steps, the negative cells passing only on the expected receipt-verified refusal; 98 permission subcells (58 executable at L3 by scripts/production_permission_cells.py, 6 evidenced by R-1, 34 BLOCKED on a task-side probe entry or the deletion role's execution path); no launch, no subcell executed, no cleanup, no AWS call, no permission granted
