...
`main`** (PR #96), the **research-build processing path is on `main`** (PR #97), a **production provider adapter is proposed in an
open pull request** (ADR-0041), and **no production image and no entry point exists**.
...
ADR-0041 production provider adapter (code):      OFFLINE / SCRIPTED-TRANSPORT-ONLY — proposed, open PR; ticker-less request form beside the unchanged accepted one
...
ADR-0041 (production provider request form):     PROPOSED — NOT IN FORCE
production ingestion/backfill/update:             NOT AUTHORIZED / NOT RUN
...
build-local mirror remains), and — until ADR-0041 is accepted — no provider adapter on `main` that can reach the
wire: the accepted request model requires a ticker on every request (ADR-0009) while ADR-0035 §3.1 designs
ticker-less cross-section requests, a conflict now resolved **in proposed offline code** by a second explicit request
form (ADR-0041, open pull request) rather than by widening the accepted model — no task-side spent-identity source (an owner decision), no R-3 tooling, no strategy
...
production provider adapter (code):                OFFLINE / SCRIPTED-TRANSPORT-ONLY — proposed in an open PR (ADR-0041); from/to inclusivity, page order and error behaviour undocumented
ADR-0038 (production run reservation):             ACCEPTED / IN FORCE — PR #96 merged 2026-09-12T15:10:12Z
