...
| Stage 3A — bounded empirical qualification, Run A and Run B | **COMPLETED ONCE EACH (2026-09-04 and 2026-09-12 UTC)** — command outcomes, not provider verdicts; provider budget 96 of 96 consumed; no retry authorized |
| Stage 3A — combined Run A / Run B assessment | **REFUSED ONCE (exit 9, zero writes — a wrong Run B identity, established offline), then COMPLETED ONCE (2026-09-12 UTC)** — 194 reads, one owner-only private report, P1–P9 not recorded in this repository |
| G1 — provider selection | **DECIDED IN PART — ADR-0034 in force (PR #92 merged 2026-09-12)**: Sharadar `tickers` and `stocks`, `actions` restricted; **OPEN** for every other domain |
| G2 — production information-set profile | **OPEN** — target `PROVIDER_REALISTIC_PIT`; closing criteria G2-A…G2-H accepted in ADR-0035, none yet met |
...
| [ADR-0043](docs/decisions/ADR-0043-production-task-entrypoint-composition.md) — production task entrypoint composition | **ACCEPTED / IN FORCE** — PR #100 merged **2026-09-12T19:55:44Z**, merge commit **`2b956c05d1aa171b571f6739d3d5756a00479847`**, approved head **`59f814ede3410a12eb84c7e02871886ec6116d17`**, merge tree identical to the reviewed head tree; **while PR #100 was open it was proposed and carried no authority** — true then, and not rewritten. A narrow completion of ADR-0036 §2.9/§2.12: two closed task entries (`kalpamani-production-acquire`, `kalpamani-research-build`, the task definitions' `command` tokens) composing the accepted bootstrap and processing paths offline through injected factories — put-only S3 and the provider adapter for acquisition, get-and-put S3 and no secret or transport for build; a task refuses a non-task credential environment by variable name, an unpinned provider origin, an incomplete compiled configuration and a factory that fails, each before any client exists; one bounded metadata v4 read of documented fields; the regional STS endpoint; one attempt in total on every task client; a receipt of allowlisted sentences and observed counts with cleanup failures beside the outcome and exit `0` only on `COMPLETED`. Its §3/§4 **proposed, and the merge did not decide**, how the secret identifier, the origin address set, the build configuration and a task-side spent-identity document reach the image, and how the ledger row is completed — those are now resolved, as a proposal of its own, by ADR-0044. No Terraform, IAM or network declaration changed; **acceptance authorized no image, launch or run** |
| [ADR-0044](docs/decisions/ADR-0044-production-delivery-contracts-and-packaging.md) — production delivery contracts and packaging | **ACCEPTED / IN FORCE** — PR #101 merged **2026-09-14T01:09:53Z**, merge commit **`98addd070857143b2bd79ef3e2bf6c06539555ba`**, approved head **`0064aa0b66d03ec3b40742ac455aa78df9074536`**, merge tree identical to the reviewed head tree; **while PR #101 was open it was proposed and carried no authority** — true then, and not rewritten. Resolves ADR-0043 §3/§4 as one design and states the amendments to ADR-0036 it needs: an image cannot know its own digest or revision, so `CompiledTask` v2 compiles the actor, family, code commit and the SHA-256 of **one closed compiled-configuration file** (secret **name** and origin address set for acquisition; the whole build configuration for build; never a credential, ARN or bucket), and the **placement release v2** binds the image digest the launch tool observed through `DescribeTasks` and the registered configuration digest, which the task compares at the barrier (`IMAGE_MISMATCH`, `CONFIGURATION_MISMATCH`); the self-check keeps family and one-image clauses only. **Acquisition input v2** carries the owner ledger's spent identities (sorted, digested, ≤ 128; a missing block is a malformed input, never an empty ledger) — no fourth parameter, no IAM change; the durable run reservation stays the guard. The receipt gains one machine-readable line (`kalpamani-task-receipt/v1`) bound to the launch record by a digest, never an identifier — emitted by every outcome, the entrypoint's own two early refusals included, with `entry`/`actor` null for an invalid invocation — with a validator whose parsing is total and closed (duplicate keys at any depth, undecodable text and wrong-type fields are closed defects, never raw exceptions) and that refuses contradictory or duplicate evidence and completes a ledger row only from observed `COMPLETED` counts; the workstation collector and its `logs:GetLogEvents` delta are proposed and deferred. Packaging prepared offline (Dockerfile with required base-digest, commit and configuration-digest arguments held equal to the copied file at build time; a build context prepared from the exact Git tree of the recorded commit by `scripts/production_build_context.py`, never from a working tree, with the compiled configuration a separately declared, digest-bound input; pinned SDK constraints; deterministic compiled-configuration generator; documented build/registration/verification procedure). **Acceptance authorized no image, launch or run**, and **the receipt collector, its `logs:GetLogEvents` IAM delta and deployment remain deferred**. A later, separately authorized **local image-verification cycle** built both actor images once, from contexts prepared from the exact tree with **synthetic** configurations, and verified their packaged entrypoints in **network-disabled** containers shaped like the task definitions (read-only root, `/work` tmpfs, user `10001:10001`) — fixing four packaging defects the accepted image demonstrated (CRLF entry executables from `git archive` on Windows, an untraversable `0444` configuration directory, a uid-999 runtime user, a working directory in a `/tmp` that a task does not have); **no image published, no digest registered, nothing run on AWS** |
| **[ADR-0045](docs/decisions/ADR-0045-verification-entries-observation-build-and-launch-tool.md) — verification entries, the observation build, and the owner-side launch tool** | **ACCEPTED / IN FORCE** — PR #104 merged **2026-09-14T17:28:56Z**, merge commit **`af20f36acbe8a3006830762fbad5cb01d1e9b38b`**, approved head **`2966ed2f24f7841eea264657f75e9c081e35922a`** (after two independently reviewed correction cycles), merge tree identical to the reviewed head tree; **while PR #104 was open it was proposed and carried no authority** — true then, and not rewritten. Completes the verification path the readiness record (PR #103) found absent: ADR-0043's two closed entries become four — `kalpamani-production-acquire-verify` and `kalpamani-research-build-verify` compose the accepted bootstrap and **nothing after `RELEASED`**, exiting `VERIFIED_BOOTSTRAP` (18, non-zero so that `0` stays `COMPLETED` alone), with factories that have no field for a secrets client, a transport or an S3 client; the build verification entry makes **one** TCP connect (443, 5 s, one attempt, to the smallest resolved address only when every resolved address is inside the compiled set) and records a closed observation, while the isolation verdict is the launch tool's — `FAILED` on `CONNECTED`, otherwise `INCONCLUSIVE`, `VERIFIED` only with a VPC Reachability Analyzer analysis matching source and destination, finding no path and naming a blocking component (its IAM delta recorded as an owner input and **not granted**; Flow Logs not admitted); **Route B**: an explicitly empty accepted-schema set admits nothing, the build refuses at normalization with zero writes, and receipt v2 (`kalpamani-task-receipt/v2`) carries the per-dataset header digests it observed as **evidence for owner review, never an accepted set**; two verification task-definition families per actor at stage a/b under their own digest keys, each launcher gaining exactly one more `ecs:RunTask` resource and no assignment change; `scripts/production_launch.py` — the owner-side launch tool — refusing by default over closed owner records (ledger, launch inputs, authorization, launch record, evidence), materializing both inputs with the accepted digest functions, compiling `CompiledLaunch` per actor and kind, checking configuration equivalence before a verification launch, composing `human_bootstrap` under the human and launcher profiles and `launch_authorized_run` on real clients built only inside its authorized branch, writing one provisional `EXIT_CODE_ONLY` ledger row per authorized identity (a `verify-` identity is a verification launch's and is consumed for both kinds; no `RunTask` is ever retried) and completing the row only from a receipt verified against the launch record; re-verification decided (R-1/R-2 repeat on a new commit or verification digest; a stage-b production rotation needs neither); corrected after the independent review of PR #104 within the same open pull request (ADR-0045 §10): the identity is reserved durably and exclusively before any external mutation and recovered rather than relaunched, the authorization binds a launch specification, every configuration file is bound to its registered target with the task definitions compared from owner-transcribed evidence, and the R-2 verdict is derived from a bound Reachability Analyzer transcription and recorded by the tool; a second correction cycle anchored reservations and the ledger lock to the ledger's canonical path (a records directory is an evidence destination only; recovery works from any of them; first-revision reservations under a records directory refuse until moved by hand) and bound the verdict to the recorded placement (the launch record carries the verified security groups and the specification digest, is bound to its reservation and ledger row, and a supplied launch-inputs file is verified against the recorded specification rather than used). **Everything offline, on fakes; nothing run; no image built; Terraform validated in an external copy and not planned or applied; G-14 kept deferred; acceptance authorizes no execution**. Since amended narrowly by §11 (PR #120: a verification-only build launch may carry an empty run set), §12 (PR #121: the held-task hook admitted for a released build verification launch) and §13 (PR #126 / ADR-0052: the hook's attachment point is the dedicated `R2-BLD-CORROBORATION` cell) |
| **[ADR-0046](docs/decisions/ADR-0046-verification-tooling-materializer-r3-tool-and-cell-runner.md) — verification tooling: the production human-binding materializer, the R-3 verification tool, and the verification cell runner** | **ACCEPTED / IN FORCE** — PR #105 merged **2026-09-14T20:37:38Z**, merge commit **`5174dcf290b4837d38af8ce4a4b975c6557e482e`**, approved head **`c4436648d90d39baa3769e990887e6f0f239df40`**, base **`af20f36acbe8a3006830762fbad5cb01d1e9b38b`**, merge tree identical to the reviewed head tree (`4ec26720c7635257a3f938b21750e9b778fc42a9`); **while PR #105 was open it was proposed and carried no authority** — true then, and not rewritten. Acceptance authorized no R-3 session, no materialization, no launch, no run, no probe, no analysis, no Terraform plan or apply, no IAM change. Owner-side tooling over accepted contracts for readiness S5, S7 and S9: `scripts/production_human_binding_materialize.py` (one authorization flag per actor, the ADR-0024 environment binding as source, the compiled profile, validated through the accepted parser before a byte is written, written by the private-artifact writer, re-read through the accepted human loader — an owner declaration, never an identity proof); `scripts/production_r3_verification.py` + `r3_verification.py` (the accepted nine rows and ten-operation cleanup on an injected client under the foundation control profile after the identity gate; every answer one closed class, the resource-based explicit-deny context the only expected denial; `kalpamani-r3-verification-record/v1` binding the declared `storage.tf` digest, the environment-binding digest, partition and region — old evidence attests to nothing changed; the digest printed only when VERIFIED; default performs nothing); `scripts/production_verification_cells.py` + `verification_cells.py` (R-1…R-9 enumerated from ADR-0036 §3; state derived from the ledger, the reservations, the verdict records, the R-3 record and the launch inputs; one identity and one authorization per cell; execute only when PREPARED with every prerequisite PASSED; interrupted cells recovered, never relaunched; bootstrap and isolation separate; aggregate VERIFIED only when every cell PASSED). **Deferred** the negative R-1 cells and R-4…R-9 orchestration (§4) — since decided by ADR-0047, accepted on the merge of PR #106. **Everything offline, on fakes; nothing run; acceptance authorized no execution**. Since amended narrowly: the `TARGET_NOT_FOUND` vocabulary addition (PR #118) and §7 (PR #126 / ADR-0052: the `R2-BLD-CORROBORATION` catalogue row, the verdict's dependency on it, and the refusal to rebind a launched cell) |
| **[ADR-0047](docs/decisions/ADR-0047-negative-verification-launches-and-permission-subcells.md) — negative R-1 verification launches and the R-4…R-9 permission subcells** | **ACCEPTED / IN FORCE** — PR #106 merged **2026-09-15T00:26:19Z**, merge commit **`167f1564378cfb96759093b43fbde5443f6c56b6`**, approved head **`7eecbb61747f8516dafd0cd44261b816fe5312bd`** (after three independently reviewed correction cycles), base **`5174dcf290b4837d38af8ce4a4b975c6557e482e`**, merge tree identical to the reviewed head tree (`5fcccc891aa52b0004b35656c25a9df3e76cc850`); **while PR #106 was open it was proposed and carried no authority** — true then, and not rewritten. Acceptance authorized no execution and granted no permission; one sentence of §3.4 is corrected after acceptance (an unverified cleanup establishes no removal, so the prerequisite object stays available — never *unavailable*), recorded beside the original. `ReleaseMode` (`NORMAL` / `WITHHELD` / `MISMATCHED`) bound into the launch specification (and so into the authorization's digest) and the launch record beside the launcher's own observed exit code; the launcher writes no release under `WITHHELD` and a release naming a derived, never-equal task ARN under `MISMATCHED`, the same sequence otherwise, no retry in any mode; the cell runner prepares, executes and completes the four negative cells and records the verified receipt's refusal (`kalpamani-negative-launch-evidence/v1`) — the expected refusal, receipt-verified and bound to its reservation, record, terminal state and the registration now in force, passes; an unexpected success, another refusal, a released or operating task is FAILED; a refused verification row is never a bootstrap success and never buildable. ADR-0036 §3's R-4…R-9 expanded into 98 subcells (`permission_cells.py`), each traced to its row's phrase — 56 executable at L3 under a human or launcher profile by `scripts/production_permission_cells.py` (one prepared statement binding the exact target, the targets document and the bound prerequisite records; one owner authorization naming it, consumed durably beside the ledger before the operation — one execution per authorization, whatever follows; one operation, one principal, one transport attempt, the attempt recorded before the operation and joined to its result by identity; every returned task of an unexpected launch stopped at once and recorded, termination confirmed only by the cleanup's `DescribeTasks`; a possibly committed write or launch recorded and settled by the cleanup, never retried; every record bound to the environment binding, the production declarations and the registration), 6 launcher positives evidenced by the R-1 bootstrap launch, 36 BLOCKED with their exact dependency on the day of acceptance (a task-side permission probe entry for the 32 task-role subcells; an execution path for the deletion role; a running task of the actor for the two `ExecuteCommand` subcells) — a human role never stands in for a task role — of which 34 are since executable offline by the mechanisms proposed ADR-0048 delivers (never run) and 2 stay BLOCKED on the deletion role's path; cleanup under the control principal by exact identity, deferring an object a prepared dependent still needs, confirmed or reported as residue; a permission cell passes only when every subcell is matched under the current binding, and empty, partial, simulated or blocked coverage never passes; a negative cell's missing evidence after an interrupted completion is recovered offline from the re-verified receipt, never relaunched. **Everything offline, on fakes; nothing run; acceptance authorizes no execution and grants no permission** |
| **[ADR-0048](docs/decisions/ADR-0048-permission-probe-tasks-held-execute-command-and-the-deletion-rehearsal-path.md) — permission-probe tasks, the held `ExecuteCommand` check, and the deletion rehearsal path** | **ACCEPTED / IN FORCE** — PR #107 merged **2026-09-15T10:22:22Z**, merge commit **`c0566574c41bc31b7144fafe919078a213982143`**, approved head **`86fe67b18cb6a213559b418d5312785972b675eb`** (after one independently reviewed correction cycle), base **`167f1564378cfb96759093b43fbde5443f6c56b6`**, merge tree identical to the reviewed head tree (`458320e33dcff5a6c579947cdc31c7bc1bc96b4b`); **while PR #107 was open it was proposed and carried no authority** — true then, and not rewritten. Acceptance authorized no execution and granted no permission. The two mechanisms ADR-0047 §5 named as required: a **permission-probe entry** per actor (`kalpamani-production-acquire-probe`, `kalpamani-research-build-probe`; a probe family beside each actor's production and verification families with the same task role, execution role and placement; one more exact `ecs:RunTask` resource per launcher) that composes the accepted bootstrap over a closed **probe input** (`kalpamani-permission-probe-input/v1`: the subcell, the bound statement and attempt digests, the stamp, the exact resolved target) and then issues **exactly one** catalogued operation under the task role through the same engine the workstation uses, over the one service that operation names, carrying the answer home as a closed receipt block (`kalpamani-task-receipt/v3`, `permission`) the workstation verifies against its launch record before it writes the permission record (`--complete-subcell --receipt-lines`; `AWAITING_RECEIPT` until then, never `PASSED` on an exit code); the launcher's **`ExecuteCommand` refusal against its own held probe task** (one check, admitted only on a fresh `RUNNING` description of exactly that task and kept as evidence, the documented `interactive: true` request, an unexpected session stopped at once, `enableExecuteCommand` stays false and the launcher's deny stays); every issuing service's documented denial classified (`AccessDeniedException`, `UnauthorizedOperation`); probe launches reserved beside the ledger before `RunTask`, tagged `startedBy` for the cleanup, in the owner ledger as `PROBED`, recoverable offline when interrupted; every probe result bound to its launch, its re-verified receipt and its ledger row (correction 1, ADR-0048 §8). Layers now 56 / 6 / 32 / 2 / 2: the **deletion role's rehearsal path is designed (§4) and NOT opened** — its 2 subcells stay BLOCKED on that governance decision (ADR-0007's verified inert property). Everything offline, on fakes and an intercepted transport; **nothing run; acceptance authorized no execution and granted no permission**; the deletion decision is presented as ADR-0049 D-1 (ADR-0049 accepted on the merge of PR #108; D-1 made concrete by ADR-0050, accepted on the merge of PR #109, and still not taken) |
| **[ADR-0049](docs/decisions/ADR-0049-receipt-collection-and-the-deletion-rehearsal-decision.md) — receipt collection from the launch's own stream, and the deletion rehearsal decision** | **ACCEPTED / IN FORCE** — PR #108 merged **2026-09-15T13:53:41Z**, merge commit **`82cae1ebfcc99dbbc53f67e534e4d78ffb914d4a`**, approved head **`9625e241db01b2996fbf9f63814ba3a916a3def7`** (after three independently reviewed correction cycles), base **`c0566574c41bc31b7144fafe919078a213982143`**, merge tree identical to the reviewed head tree (`df0451e43b0b21a1e7659b88cbc7abe82366119c`); **while PR #108 was open it was proposed and carried no authority** — true then, and not rewritten. Acceptance decided D-1 in neither direction, authorized no execution and granted no permission. A **bounded receipt collector** that derives a launch's log stream (`<prefix>/<container>/<task-id>`) from the bound launch record and one optional registered block of the task-definition evidence (`log_destination`), never from a caller; reads it with `GetLogEvents` under bounds checked before every request (40 requests, 20,000 events, 300 s; 16-page passes; 15 s polls; effective SDK retries zero); establishes one line only from a complete scan (end token, one poll interval, a confirming re-read — the observation window; delayed delivery the stated limitation); verifies the candidate against the launch record before keeping it (a refused line leaves a closed defect and byte count, never text); keeps only the verified receipt line in a closed collection record (`kalpamani-receipt-collection/v1`) admitted by one cache rule in both tools (contradictions refuse whatever the filename order; a recorded contradiction is never superseded and is disposed only by a hand-read completion that acknowledges it by digest, recorded beside it, after which only that receipt completes the launch; rejected attempts never block); and hands it to **exactly the hand-read completion** (`--complete-row --collect-receipt`, `--collect-receipt <subcell>`, each behind its own flag and the launcher identity) — an exhausted budget proves only that nothing was obtained within it, a successful read is not a verification, and a FAILED or INCONCLUSIVE reading is preserved. The `logs:GetLogEvents` delta for the launcher sets is **recorded and not granted**. The **deletion rehearsal path** ADR-0048 §4 designed is **implemented offline and CLOSED** (`deletion_rehearsal.py`: the target derived from the bound R-4 human `PutObject` record's synthetic object and nothing else, statements and consumed authorizations, the `R8-GET` then `R8-LIST-AND-DELETE` sequence under the actual deletion role, PASS / FAIL / INCONCLUSIVE with the control principal's confirmation, residue), the two R-8 subcells stay BLOCKED, and **Decision D-1 — open the path — is presented with its consequences and taken by nobody here**; since made concrete for the owner's separate decision by ADR-0050 (accepted on the merge of PR #109; D-1 still not taken). Everything offline, on fakes and an intercepted transport; **nothing run; acceptance authorized no execution and granted no permission** |
| **[ADR-0050](docs/decisions/ADR-0050-deletion-rehearsal-decision-made-concrete-and-offline-integration.md) — the deletion rehearsal decision made concrete, its offline integration, and its inert declaration** | **ACCEPTED / IN FORCE** — PR #109 merged **2026-09-15T17:36:51Z**, merge commit **`5f5035fecef53126058e2aa56a5b82c4d036af00`**, approved head **`4ea7e73383585a22543a2cee1284a746f4d4a492`** (after two review corrections, §8), base **`82cae1ebfcc99dbbc53f67e534e4d78ffb914d4a`**, merge tree identical to the reviewed head tree (`3269a33473338805e8ba483e8d9065ce840caa2f`); **while PR #109 was open it was proposed and carried no authority** — true then, and not rewritten. **Acceptance decided D-1 in neither direction, authorized no execution and granted no permission**: D-1 stays the owner's separate written decision and is **NOT TAKEN**; the rehearsal path stays CLOSED, `deletion_rehearsal_open` stays `false`, both R-8 subcells stay BLOCKED. **Decision D-1 made concrete** (§2): the exact resources and permissions (a `kalpamani-deletion-rehearsal` task definition whose task role is the **actual** deletion role and whose only entry is the rehearsal's; the deletion role's delta of three exact `ssm:GetParameter` reads and a scoped `kms:Decrypt` — **no S3 change**; a runtime-binding parameter; the `KalpaManiDeletionRehearse` launcher — one exact `RunTask`, `PassRole` of exactly the deletion and execution roles, create-only input and release parameters, `GetLogEvents` on exactly the rehearsal streams, **no S3 action**; two gated key-policy statements; one assignment at stage b), who launches and which role deletes (a human launcher passes; the deletion role deletes as the task; no human ever assumes it), how the target is held to the one synthetic R-4 object (the statement, the input's recomputed digest, the task's bucket check, the engine's exact operations — **not IAM**: the deletion role's existing `s3:DeleteObject` authority is every object of the licensed bucket and the rehearsal adds, widens and removes no S3 statement), the operation limits, every interruption's recorded outcome with no retry, the control principal's cleanup as the only confirmation, the residual risks (ADR-0007's inert property reversed by design; the role's bucket-wide delete authority bounded by the image's code) and what acceptance would enable versus what stays separately authorized — **presented verbatim for the owner's signature and taken by nobody here**. The **offline integration** (§3): the rehearsal task (`deletion_rehearsal_task.py`: entry, credential environment, metadata, binding, input, target, identity, release, then the engine; exit 50–59; one bound receipt line), the launcher and completion (`deletion_rehearsal_launch.py`: identity, durable consumption, reservation, create-only input, one never-retried `RunTask`, placement, release, observation, record; the record rebuilt only from a verified REHEARSED receipt whose exit the launcher observed), the collector reused with an injected verifier, and the owner tool's five modes (`--prepare-rehearsal`, `--rehearse-deletion`, `--collect-rehearsal-receipt`, `--complete-rehearsal`, and since correction 1 `--recover-rehearsal-launch`) — every one refused with exit 27 before any path, flag or client while `REHEARSAL_PATH_OPEN` is `False`, which it is. The **inert declaration** (§4): `production_deletion_rehearsal.tf`, every resource gated on stage a/b **and** `deletion_rehearsal_open` (**false by default**) **and** a `deletion_rehearsal` image digest, validated only in an external copy under the mock provider (20 runs; the closed default declares nothing). The **owner checklist** (§5; `docs/operations/production-owner-checklist.md`), every value MISSING. Everything offline, on fakes; **nothing run; acceptance decides D-1 in neither direction, authorizes no execution and grants no permission** |
| **[ADR-0051](docs/decisions/ADR-0051-exploratory-hindsight-research-profile-and-isolation.md) — an exploratory hindsight research profile, kept apart from production qualification** | **ACCEPTED / IN FORCE** — PR #111 merged **2026-09-16T02:10:11Z**, merge commit **`bc16801efbfd9d8c5a9b947f888da64d7dadf72a`**, approved head **`56871f69f1e4443b638bf9da7c64f21f9ddafbbd`** (after review correction 1), merge tree identical to the reviewed head tree; **while PR #111 was open it was proposed and carried no authority** — true then, and not rewritten. Acceptance took the vocabulary and isolation decision only: it selected none of O-1…O-11, authorized no real-data run and implies nothing about PR #112. Defines a **research-only** profile `EXPLORATORY_HINDSIGHT` and derivation `AS_DATED` in their own vocabulary (`kalpamani.data.exploratory`) — never members of the accepted `InformationSetProfile` / `ProviderBoundDerivation`, so no accepted result, gate or production build can carry them; **assumed** historical availability distinguished from **evidenced** availability (production P-2/P-3 unchanged); mandatory declared limitations (revision look-ahead, current-attribute look-ahead, optimistic terminal valuation, no intraday instant; event blindness and benchmark self-reference when they apply); **no exploratory result satisfies P1–P9, any G2 criterion or any promotion criterion**; four closed contracts (provenance, publication, input set, research specification) parsed totally, with production values refused by name as substitutions and mixed provenance refused; admission by exact-type opt-in only — the accepted Breakout Long specification and every other consumer are refused; acceptance kept apart from execution. **Implemented offline beside it, synthetic documents only**; a later, separately authorized slice (§9) implemented — **offline, on synthetic fixtures only** — the AS_DATED resolution and membership, a Silver-to-exploratory bridge (never a `VerifiedPublication`), benchmark A, the M0 runner (`breakout-long-m0-exploratory-v1`, the accepted module and thresholds unchanged, final-fill rejection, prior-session ADDV, causal terminal recognition, two reconciled terminal ledgers, frozen trial digest), and a report labelled SYNTHETIC / EXPLORATORY_HINDSIGHT, with the §14 settings carried as a **synthetic fixture, not owner selections**, and a real-data run refused without a typed, complete, supported `OWNER_SELECTED` configuration; **review correction 1 of that path (§10, within PR #112 while it was open)** made price bases consistent across splits with an explicit fractional-share policy, closed purge/tail entries and same-session re-entry, validated every setting and typed every owner selection, bound strategy identity and calendar content into the trial record, labelled evaluation and liquidation-inclusive periods with identical benchmark instants per terminal policy, and added a transaction journal reconciled independently; **correction 2 (§11)** settles a split's fractional entitlement at an observed close and never at the open (an engineering assumption pending an owner selection; unresolved entitlements carried at zero) and restores O-7 = events / O-10 = sizing and sequencing in the typed selections; a synthetic run establishes **software behaviour only**; **slice 3 (§12)** implements the exploratory **adapter** — an admitted build's manifest and Silver artifacts, as bytes, digest- and binding-verified (configuration digest, provenance runs, `served` excluded-by-time = 0, 252-session rule) into an `ExploratoryPublication`, proven on artifacts from the accepted producer, reading no store and no licensed object; **review correction 1 (§13)** binds dataset construction to the digest-bound configuration (`BoundConfiguration`; substituted calendar or rule `CONFIGURATION_INCONSISTENT`) and validates the whole manifest schema and its fixed vocabulary (`MANIFEST_VALUE_UNSUPPORTED`); **review correction 2 (§13.1)** reconciles the one repeated fact correction 1 left out — the evidence version, carried on `BoundConfiguration` and compared again at construction — and corrects correction 1's regression accounting (69 run against the head, 78 committed); a real-data read path and principal (O-6) do **not** exist; **PR #112 MERGED 2026-09-16T10:18:46Z** (merge commit `88053d378bb8391d6cee8be22b2b94e9c0632083`, ordered parents `bc16801efbfd9d8c5a9b947f888da64d7dadf72a` then `bf8bd18718bb3bad89602e7a108ddb0677342eb6`, merge tree identical to the reviewed head tree — the synthetic M0 path is on `main`; while PR #112 was open it was unmerged, true then and not rewritten); slice 3 and its two corrections are carried by **PR #113**; benchmark, acquisition route, compute and every other M0 decision **pending**; real-data backtesting **NOT STARTED**; D-1 undecided; rehearsal CLOSED |
| **[ADR-0052](docs/decisions/ADR-0052-dedicated-r2-corroboration-cell.md) — a dedicated R-2 corroboration cell; the reachability hook rides its own launch; a PASSED cell is never rebound** | **ACCEPTED / IN FORCE** — PR #126 merged **2026-09-17T18:53:23Z**, merge commit **`01f77a10a2341b1d1d99a7fe0024b104563d692c`**, approved head **`a489f13f277b12109af5b792c6616f7327be495d`**, base **`d5407d8a84538827730e576240e9ec0023997671`** (the PR #125 merge), merge tree identical to the reviewed head tree (`a207f4146dba274e8294f87bbe26f44b4ee9ee7f`); **while PR #126 was open it was proposed and carried no authority** — true then, and not rewritten. Acceptance authorized no execution. Adds one cell to the ADR-0046 §2.3 catalogue, **`R2-BLD-CORROBORATION`** (R-2, runtime launch, build actor, `kalpamani-research-build-verify`, requires `R3` and `R1-BLD-BOOTSTRAP` PASSED; executed by the accepted cell runner and launch tool like the bootstrap cells over the accepted empty run set with a `NORMAL` release; its PASSED is the launch's own success and never the isolation verdict); re-attaches ADR-0045 §12's `while_running` hook to that cell alone (ADR-0045 §13); makes `R2-BLD-ISOLATION` depend on it and take its verdict on the corroboration launch's record with the accepted binding checks unchanged; and has `--prepare-cell` refuse (`refused_cell_state`) any cell whose bound identity was launched, so the PASSED `R1-BLD-BOOTSTRAP` cell's binding, reservation, record, receipt and row stay byte for byte (ADR-0046 §7). R-2 PASSED only from actual returned path + analysis evidence the owner attests (D-16), bound to the corroboration launch's own interface, window and destination; missing, mis-attributed, out-of-window, wrong-destination, failed, path-found or cleanup-incomplete evidence is INCONCLUSIVE or the closed failure, never a pass. Everything offline, on fakes (the end-to-end lifecycle with its negative controls); **owner acceptance of the design (2026-09-17) waives, weakens and marks nothing** — R-2 still requires the launch, attribution, an in-window analysis, matching returned evidence, `network_path_found = false`, an admitted explanation on a placed component, complete cleanup, the owner's D-16 and the `--verdict-cell` step. State on the day of this row: the cell **PREPARED, not executed** (identity `verify-bld-20260917T185351Z-9a3a7ce0`, specification `b93d656d…`, under registration `a0155d0a…`); its one execution and the verdict are separate written authorizations |
| **[ADR-0053](docs/decisions/ADR-0053-pagination-v2-single-data-page-and-completion-probe.md) — pagination v2: one governed data request plus one completion probe per group, multi-page data refused on evidence, qualified ceilings, and the whole-run replacement of acquisition run 1** | **ACCEPTED / IN FORCE** — PR #129 merged **2026-09-18T02:01:09Z**, merge commit **`3e8c9cb5c13141c0353d4bb62c3e65c498badf1b`**, approved head **`cef2db911e346630e3d04306c292ffaa389c7429`**, base **`7cfb9bac4676a724e9265a7ff34cb39641b568da`** (the PR #128 merge), merge tree identical to the reviewed head tree (`816eb0ca5f24866f682a4ccda54df8cdb2d565f7`); **while PR #129 was open it was proposed and carried no authority** — true then, and not rewritten. Accepted as **governance and contract only** (the owner's verbatim route decision of 2026-09-18, the 13-point pagination-v2 contract, the qualification gate, the replacement and planning consequences, the corrected deployment-impact statement and the dated amendments of ADR-0009, ADR-0041, ADR-0042, ADR-0035, ADR-0040 and ADR-0043) and authorizes nothing that runs — nothing is qualified by the acceptance. Evidence: the first production acquisition (O-5 run 1, `run-20260917T221453Z-af316f8b`, 2026-09-17, `COMPLETED` exit 0, receipt-verified — 96 provider requests, 1 secret retrieval, 290 conditional writes; locator admitted) delivered **every tickers and actions page full at 10,000 rows** (responses continue beyond the compiled ceilings) and **2,147 byte-identical rows on two tickers offsets** — offset assembly is neither complete nor deterministic — so run 1 is **historical evidence, not buildable**, to be replaced whole (≈ 94 requests) after a **separately authorized qualification cycle** fixes the limit `L` and every byte/row/memory ceiling (**none is qualified by the ADR**; the 16 MiB response ceiling is visibly insufficient and no replacement is invented; the fallback is a deterministic server-side partition, never offset pagination). Run 2's prepared specification (`899f2e11…`) is superseded and preserved with its identity unconsumed and no D-15; runs 2–19 on hold; S10c blocked; four images, four release-bound configuration digests, four task definitions, both launcher policies and four registration blocks expected to move, with the S9/permission cells to repeat rederived from the binding rules. **D-19 attempt 1 (2026-09-18) → `PROVIDER_REFUSED`, preserved**: the tickers data page at `limit=100000` is complete-shaped in itself (74,202 rows, 25.4 MB) but an unfiltered tickers request mixes the vendor's table namespaces (30,304 conflicting `permaticker` repeats), the probe at `skip=100000` was refused HTTP 400, actions untested, peak RSS unmeasured — `L = 100000` **not qualified**. **§11 amendment (proposed in the pull request introducing it; effective on its merge)**: conditional completion (a data page below `L` needs no probe; exactly `L` rows require one parsed empty probe), an explicit documented `table` predicate per tickers group with `permaticker` the within-group identity and tables never combined — the accepted predicate set for the first observation build is exactly `table=stocks`, derived from the accepted consumers (§11.4) — and the second qualification as owner-input D-20. **D-20 round 2 (03:14Z) → `DUPLICATE_PRIMARY_KEY`, preserved**: the `table=stocks` tickers group complete-shaped without a probe (20,976 rows / 8.2 MB; one table value; unique `permaticker`), actions window 1 41,437 rows with 44 coarse-key repeats of differing content, window 2 untested — `L = 100000` still not qualified. **§12 amendment (proposed in the pull request introducing it; effective on its merge)**: a schema-bound canonical full-row actions event identity (`sharadar-actions-event-identity/v1`: the seven governed fields `date, action, ticker, name, value, contraticker, contraname` in accepted order — derived from the accepted Route-A digest — typed normalization, exact duplicates refused, no precedence, no silent combining, deterministic ordering, schema-bound) replacing the coarse key; group-scoped reuse of the D-20 tickers group under a strict binding; the third qualification as owner-input D-21. **D-21 round 3 (06:02Z) → `QUALIFIED_AT_L_100000`** — the first successful overall qualification (actions 41,437 / 46,295 rows, 0 duplicates, no probe; tickers reused; clean memory). **§13 amendment (proposed in the pull request introducing it; effective on its merge)**: the owner accepts the envelope as **offline implementation targets only** (`L` 100,000 tickers/actions; stocks 10,000; 32 MiB whole-body ceiling, never truncation; 1,024 MiB process; task 2,048 MiB / 30 s / 1,800 s unchanged), the conditional-probe evidence model (one data coordinate per group; a probe is a bounded verification, never a payload; writes `1 + 3N + 1`; a `2N` call ceiling), and arithmetic marked CALCULATED — PLANNER MUST RECOMPILE (run 1′ ≈ 47 / 143; O-5 ≈ 10 / 847 / 2,561); D-19 and D-20 stay failed attempts; nothing deployed |
| **[ADR-0054](docs/decisions/ADR-0054-historical-v1-reservations-and-registration-historical-rebinding.md) — historical v1 reservations readable as evidence, and registration-historical rebinding under a preserved, digest-bound supersession** | **ACCEPTED / IN FORCE** — PR #135 merged **2026-09-18T17:35:07Z**, merge commit **`8be462b953c33a6953429ee9edc321f843ab6ae1`**, approved head **`bbaa419dea40f53040f2d228d25fc1eb631b713c`**, base **`a5a6384f44df09cbf7736e2ec0e58d9c5e0149bc`** (the PR #134 merge), merge tree identical to the reviewed head tree (`39a7eac736d63eeacde27a853fe0f48aaa078fab`); **while PR #135 was open it was proposed and carried no authority** — true then, and not rewritten. Accepted as **two narrow governance decisions and their amendments only** (the owner's verbatim decisions of 2026-09-18): (1) a supported, integrity-valid v1 reservation — an acquisition workload compiled under the superseded `kalpamani-production-acquisition-plan/v1` contract — may be read by pagination-v2 workstation tooling **solely to preserve and classify historical evidence** through a distinct evidence-only parser (`HistoricalReservation`; strict parse first, the plan digest never recompiled, a current document refused `WORKLOAD_CURRENT`, every other v1 shape, corrupted or orphaned reservation still refused, no identity or filename special-cased, original bytes untouched), never becoming launch authority and never bypassing the v2 parser — every execution path of the launch tool still reads only the strict `reservation()`; (2) a verification cell whose prior bound launch the runner derives `HISTORICAL` **solely because the registration in force no longer names its target** may receive a fresh `verify-` identity and specification, proven before any write (launched; reservation, launch record and ledger row integrity-valid and bound; derived `HISTORICAL` for that identity and digest; registration difference re-checked; replacement identity fresh everywhere; the launch tool's admission passed), the previous cells document preserved byte for byte as an owner-only superseded copy and the new binding carrying a closed digest-bound `supersedes` link — a `PASSED`, `PREPARED`, unlaunched, malformed, same-registration or merely `BLOCKED` binding is never replaced, no prior record is deleted or rewritten, and historical evidence never becomes a current `PASSED`. Narrow dated amendments of ADR-0052 §2.3, ADR-0045 §6 and ADR-0046 §2; ADR-0053 §13.5 applied. **Deployment impact: none** — no task entry imports the changed modules; the four published pagination-v2 images, task definitions, policies and the registration `3cf07d01…` are unchanged and remain valid. Acceptance authorizes no reconciliation, preparation, launch, D-15, receipt, verdict, registration, image, IAM, Terraform or AWS/provider call — each its own written authorization (the store was reconciled and six fresh R-1 cells prepared under the prior authorization, on 2026-09-18, with nothing launched) |
| **[ADR-0055](docs/decisions/ADR-0055-compact-build-input-v2.md) — the compact, digest-bound research-build input (version 2)** | **ACCEPTED / IN FORCE** — PR #138 merged at its exact reviewed head (merge commit, parents and tree recorded in the S10c evidence packet); **while PR #138 was open it was proposed and carried no authority** — true then, and not rewritten. Accepted as **one contract decision, two refusal-boundary corrections and their amendments only** (the owner's verbatim Option A decision of 2026-09-19, owner-input D-23): the build input parameter carries `kalpamani-research-build-input/v2` (`schema_version` 2), each row exactly `{run_identity, locator_sha256}`, the ledger digest over the rows as delivered (the whole program one binding), at most 32 distinct identities and 32 distinct digests, every other row shape `ROW_MALFORMED`; the owner-side launch tool holds every preserved locator (`<ledger dir>/locators/`) to its true ledger row through the unchanged accepted validator before recording its SHA-256 (`bind_run_locators`); the task derives the locator key from the identity, holds the retrieved bytes' full SHA-256 to the row's digest in constant time **before decoding**, holds the declared `run_id` to the identity, re-derives the ledger-row view from the validated locator's own slice, plan digest and instants, and runs the unchanged validator; the slice, plan digest, outcome and instants are removed from the row **only** because the identity, the key or the validated locator determines each (field-by-field mapping in ADR-0055 §2.2); the 8 KiB ceiling is applied by the materializer **before any write** and by the task before parsing (`MAX_BUILD_INPUT_DOCUMENT_BYTES = MAX_ADVANCED_PARAMETER_BYTES`), and the launch tool reports an oversized input as one closed refusal (`refused_input_size`, exit 20, no traceback, no file, no reservation, no row, no client) — the two defects the S10c stop recorded; a 32-run document at every field's widest valid width is **5,717 bytes** (margin 2,475), so the accepted 32-run ceiling is reachable inside the **unchanged** 8 KiB tier and `MAX_BUILD_RUNS` is not reduced; `kalpamani-research-build-input/v1` is **historical evidence only** — a distinct evidence-only reader, refused by every execution path — and the 8,266-byte eighteen-run v1 preparation document of 2026-09-19 is preserved byte for byte, marked superseded for new execution, its unconsumed identity never reused. Narrow dated amendments of ADR-0036 §2.6/§2.4 clause 1 (ADR-0036 §7) and ADR-0045 §6/§7 (ADR-0045 §16); ADR-0044 unamended; no acquisition, build, point-in-time, schema, completion, write-order or audit requirement weakened. **Deployment consequences stated, not performed** (ADR-0055 §6): the build and build-verification images rebuilt from the merge tree, two new configuration digests, two task-definition replacements, the build launcher policy moved, two registration blocks re-issued, the current build-family R-1/R-2 cells left to the runner to derive; the acquisition images unchanged. Acceptance authorizes no image build, registry publication, Terraform, tfvars, registration change, AWS task, S3 operation, production build, backtest or trade — each its own written authorization |
| **Production verification tooling corrections and the R-2 corroboration path — PRs #115–#126** | **MERGED (2026-09-16 → 2026-09-17), every merge at its exact reviewed head with the merge tree equal to the head tree; no ADR superseded.** **#115** `7fcb1d58…` each launcher permission set granted `logs:GetLogEvents` on exactly its own receipt streams (ADR-0049 §2.6 delta, declared); **#116** `b5485e00…` the KMS deny scope stated without an implied-deny claim; **#117** `a59ad2c4…` the permission probe's `PutObject` sends SSE-S3 like the accepted store; **#118** `696749f6…` `TARGET_NOT_FOUND` named in ADR-0046's vocabulary and explained in ADR-0047 (an ECS target-validation answer that decides nothing); **#120** `d5349d95…` ADR-0045 §11 — a verification-only build launch may carry an empty run set (this merge is the release the six registered task definitions were built from); **#121** `1c707d30…` ADR-0045 §12 — the launcher's held-task hook admitted for a released build verification launch; **#122** `b6e544de…` the R-2 reachability watcher (`reachability_hook.py`: exact-task attribution, invocation token and tag, private journal, bounded polling, tag-filtered recovery; a proposal never attested); **#123** `ebe36468…` the launch specification parser admits the empty run set for the verification kind; **#124** `1e7c4278…` the watcher journals closed `Class:Code` failure codes; **#125** `d5407d8a…` the watcher's path request scoped at the source (`FilterAtSource.DestinationAddress` + `DestinationPortRange 443..443`; no `Destination`/`DestinationIp`/`DestinationPort`/`SourceIp`) after CloudTrail established the 16:09Z `Client.MissingParameter` refusal, with the D-16 destination transcribed from the returned path; **#126** `01f77a10…` ADR-0052 (above). **PR #119** (ADR-0047 §12, the six invalid-target R-6 subcells BLOCKED with dependencies; D-17) stays **OPEN and undecided**. Runtime evidence recorded outside this repository under these merges: registration `a0155d0a…` (six targets at revision 2 from release `d5349d95`), R-3 PASSED, the six R-1 cells PASSED under that registration (2026-09-17), the one R-2 watcher attempt `PATH_NOT_CREATED` (cause established, corrected by #125), and `R2-BLD-CORROBORATION` PREPARED and not executed. Every step was its own written authorization; nothing here authorizes the next |
...
### The assessment runtime binding, and ADR-0025 — ACCEPTED on merge; the binding since materialized; the assessment since completed

> **HISTORICAL — this section was written while PR #69 was open, and it records that day.** ADR-0025 is
> **ACCEPTED / IN FORCE** (PR #69 merged 2026-09-04T17:48:38Z); the real assessment runtime binding was
> **MATERIALIZED on 2026-09-05** under its own authorization; and the combined assessment has since been
> **REFUSED once and COMPLETED once on 2026-09-12**. Every "PROPOSED", "NOT MATERIALIZED", "NEVER RUN",
> "NOT RUN" and "UNEVALUATED" line below describes 4 September 2026 and **no longer governs**; *The completed
> Run B acquisition and the completed combined assessment* governs. The forward authorization boundaries are
> unchanged.
...
ADR-0043 task entrypoints (code):                 OFFLINE / SYNTHETIC-ONLY / NO IMAGE / NEVER RUN — on main, PR #100 merged 2026-09-12T19:55:44Z; two closed entries composing the accepted paths
ADR-0044 delivery contracts + packaging (code):   OFFLINE / SYNTHETIC-ONLY / LOCAL VERIFICATION IMAGES ONLY / NEVER RUN AS A TASK — on main, PR #101 merged 2026-09-14T01:09:53Z; CompiledTask v2, release v2, acquisition input v2, compiled-configuration file, receipt line on every outcome and a total validator, Dockerfile, generator and tree-exact build-context preparer; compiled configuration file absent by design
...
production readiness record (after PR #102):       PREPARED AND MERGED — PR #103 merged 2026-09-14 (merge commit 8464128a9d890a5d43c2a685927742cad8428f8f); docs/operations/production-readiness.md; every production owner input MISSING; receipt collector DEFERRED; nothing run
ADR-0045 verification entries, observation build, launch tool (code):   OFFLINE / SYNTHETIC-ONLY / FAKES ONLY / NEVER RUN — on main, PR #104 merged 2026-09-14T17:28:56Z (merge commit af20f36acbe8a3006830762fbad5cb01d1e9b38b; ADR-0045 ACCEPTED / IN FORCE; acceptance authorized no execution); four closed entries (two verification entries stopping at the barrier, VERIFIED_BOOTSTRAP exit 18), the build probe with a closed observation and a verdict decided outside the task, Route B observation build (empty accepted set, zero writes, receipt v2 evidence), scripts/production_launch.py refusing by default over closed owner records, two verification task-definition families and one more RunTask resource per launcher (validated offline in an external copy; NOT PLANNED, NOT APPLIED); no image built, no launch, no probe, no AWS call; G-14 bucket-policy transition KEPT DEFERRED; Reachability Analyzer IAM delta RECORDED, NOT GRANTED; PR #104 review corrections applied in the same open pull request (durable exclusive reservation before any external mutation, locked atomic ledger writes, recovery without relaunch; the authorization binds a launch specification digest; equivalence bound to every registered target with transcribed task-definition evidence, ecs:DescribeTaskDefinition read-back RECORDED AS A DEPENDENCY, NOT ADDED; R-2 verdict derived from a closed Reachability Analyzer transcription bound to the task's keyed destination digest, interface and window, recorded by the tool; second cycle: reservations and the ledger lock anchored to the ledger's canonical path with a records directory an evidence destination only and first-revision reservations refused until moved by hand, the launch record carrying the verified security groups and the specification digest, completion and the verdict bound to the reservation and ledger row, a supplied launch-inputs file verified against the recorded specification and never used for the placement) -- still offline, still fakes only
ADR-0046 verification tooling (code):                            OFFLINE / SYNTHETIC-ONLY / FAKES ONLY / NEVER RUN — on main, PR #105 merged 2026-09-14T20:37:38Z (merge commit 5174dcf290b4837d38af8ce4a4b975c6557e482e; ADR-0046 ACCEPTED / IN FORCE; acceptance authorized no execution); the production human-binding materializer (G-4; one flag per actor; materializes an owner declaration, verifies no identity), the R-3 verification tool (G-5; the nine accepted rows and the ten-operation cleanup on an injected client; kalpamani-r3-verification-record/v1 bound to the declared statements and the environment binding; default performs nothing) and the verification cell runner (G-6; cells enumerated from ADR-0036 §3, state derived from the ledger, reservations and records; the negative R-1 cells and R-4…R-9 since decided by proposed ADR-0047); no R-3 session, no materialization, no launch, no AWS call
ADR-0047 negative launches + permission subcells (code):         OFFLINE / SYNTHETIC-ONLY / FAKES ONLY / NEVER RUN — on main, PR #106 merged 2026-09-15T00:26:19Z (merge commit 167f1564378cfb96759093b43fbde5443f6c56b6; ADR-0047 ACCEPTED / IN FORCE; §3.4's prerequisite sentence corrected after acceptance); ReleaseMode on the specification and the record, the launcher's WITHHELD and MISMATCHED steps, the negative cells passing only on the expected receipt-verified refusal; 98 permission subcells (56 executable at L3 by scripts/production_permission_cells.py under a prepared statement and a consumed per-subcell authorization, 6 evidenced by R-1, 36 BLOCKED on a task-side probe entry, the deletion role's execution path or a running task for ExecuteCommand); PR #106 correction 1 (authorization consumption, prerequisite identity, possibly committed writes and launches settled by identity, ECS task accounting, negative-evidence recovery) and correction 2 (one chain validator over attempt / statement / exact target / consumption against the context in force incl. the targets document; bounded ECS discovery, termination evidence only) and correction 3 (ListTasks by startedBy alone as the contract requires, request shape held at the intercepted transport, stopped-task visibility a stated limitation; an unverified cleanup identity settles nothing) offline; no launch, no subcell executed, no cleanup, no AWS call, no permission granted
ADR-0048 permission-probe tasks + held ExecuteCommand (code):     OFFLINE / SYNTHETIC-ONLY / FAKES ONLY / NEVER RUN — on main, PR #107 merged 2026-09-15T10:22:22Z (merge commit c0566574c41bc31b7144fafe919078a213982143; ADR-0048 ACCEPTED / IN FORCE); two probe entries and families, the probe input contract, receipt v3 with the permission block, LaunchKind permission-probe and the PROBED ledger outcome, the tool's probe launch and --complete-subcell, the launcher's one while-running ExecuteCommand check against its held probe task (admitted only on a fresh RUNNING description of that task, kept as evidence; interactive: true, an unexpected session stopped), every issuing service's denial classified; after review correction 1: probe launches reserved beside the ledger before RunTask, offline recovery of an interrupted launch and a repeatable completion, and a validator requiring the launch, the re-verified receipt and the ledger row of every probe result; layers 56 / 6 / 32 / 2 / 2 — the deletion rehearsal path designed and NOT opened (R-8 stays BLOCKED); Terraform probe families and launcher RunTask resources declared and validated in an external copy only; no probe launched, no subcell executed, no image, no plan/apply, no AWS call, no permission granted
ADR-0049 receipt collection + deletion rehearsal (code):          OFFLINE / SYNTHETIC-ONLY / FAKES ONLY / NEVER RUN — on main, PR #108 merged 2026-09-15T13:53:41Z (merge commit 82cae1ebfcc99dbbc53f67e534e4d78ffb914d4a; ADR-0049 ACCEPTED / IN FORCE; acceptance decided D-1 in neither direction, authorized no execution and granted no permission); the bounded receipt collector (stream derived from the bound launch record and the registered log destination; 40 requests / 20,000 events / 300 s checked before each request, SDK retries zero; one line established only by a complete, confirmed scan and verified before it is kept; the closed collection record keeping the verified receipt line only, admitted by one cache rule in both tools, a recorded contradiction never superseded (explicit acknowledged disposition only, binding the launch to its receipt); completion through the hand-read path behind its own flag and the launcher identity; corrections 1-3 after review; logs:GetLogEvents recorded, not granted); the deletion rehearsal path implemented offline and CLOSED (REHEARSAL_PATH_OPEN false, R-8 stays BLOCKED, the tool refuses --rehearse-deletion), Decision D-1 presented and not taken; ADR-0048 synchronized to ACCEPTED / IN FORCE; no log read, no collection, no rehearsal, no deletion, no image, no plan/apply, no AWS call, no permission granted
ADR-0050 deletion rehearsal readiness (code + declaration):       OFFLINE / SYNTHETIC-ONLY / FAKES ONLY / NEVER RUN — on main, PR #109 merged 2026-09-15T17:36:51Z (merge commit 5f5035fecef53126058e2aa56a5b82c4d036af00, approved head 4ea7e73383585a22543a2cee1284a746f4d4a492, merge tree = reviewed head tree; ADR-0050 ACCEPTED / IN FORCE per its own clause; while PR #109 was open it was proposed and carried no authority); acceptance decided D-1 in neither direction, authorized no execution and granted no permission; Decision D-1 (ADR-0049 s.3.8) made concrete for the owner's separate signature and NOT TAKEN; the rehearsal task under the actual deletion role (exit 50-59, one bound receipt line), the launcher and completion (durable consumption, reservation, one never-retried RunTask, placement, release, observation, record; completion only from a verified REHEARSED receipt whose exit the launcher observed), the collector reused with an injected verifier, five owner-tool modes refused with exit 27 while REHEARSAL_PATH_OPEN is False (it is); correction 1 after review (ADR-0050 s.8, same open pull request): reservations and unresolved-launch resolutions anchored beside the canonical ledger, every launch refused while one is unsettled (settled only by the accepted cleanup rule -- a listing that finds nothing settles nothing), offline recovery (--recover-rehearsal-launch), one evidence-binding rule for completion and prerequisite admission over the exact target, the retained verified receipt, ADR-0049's contradiction/disposition rules on the hand path; correction 2 (ADR-0050 s.8.4): a StopTask acknowledgement settles nothing -- the exact task is observed within the one bound, STOP_ACKNOWLEDGED stays unsettled with the task identity kept until the cleanup describes it STOPPED; production_deletion_rehearsal.tf declared INERT (deletion_rehearsal_open false by default; stage a/b and an image digest required; validated in an external copy under the mock provider, 20 runs); the owner checklist (production-owner-checklist.md), every value MISSING; ADR-0049 synchronized to ACCEPTED / IN FORCE; no rehearsal, no deletion, no launch, no log read, no image, no plan/apply, no AWS call, no permission granted, no variable set
ADR-0052 dedicated R-2 corroboration cell (code):               ON MAIN — PR #126 merged 2026-09-17T18:53:23Z (merge commit 01f77a10a2341b1d1d99a7fe0024b104563d692c, approved head a489f13f277b12109af5b792c6616f7327be495d, merge tree = reviewed head tree; ADR-0052 ACCEPTED / IN FORCE per its own clause; while PR #126 was open it was proposed and carried no authority); R2-BLD-CORROBORATION PREPARED and NOT executed (identity verify-bld-20260917T185351Z-9a3a7ce0, specification b93d656d…); R2-BLD-ISOLATION BLOCKED on it; the hook rides that cell alone; the PASSED R1-BLD-BOOTSTRAP is never rebound; acceptance authorized no execution
ADR-0053 pagination v2 (governance):                             ACCEPTED / IN FORCE — PR #129 merged 2026-09-18T02:01:09Z (merge commit 3e8c9cb5c13141c0353d4bb62c3e65c498badf1b, approved head cef2db911e346630e3d04306c292ffaa389c7429, merge tree = reviewed head tree; while PR #129 was open it was proposed and carried no authority) — governance and contract only; no limit or ceiling qualified; D-19 attempt 1 PROVIDER_REFUSED 2026-09-18 (L=100000 not qualified; preserved); §11 amendment (conditional probe; tickers table=stocks predicate) PROPOSED in its pull request; D-20 round 2 DUPLICATE_PRIMARY_KEY 2026-09-18 (tickers table=stocks group COMPLETE; actions coarse key too coarse; L=100000 not qualified; preserved); §12 amendment (canonical full-row actions identity) PROPOSED in its pull request; D-21 round 3 QUALIFIED_AT_L_100000 2026-09-18 (the first successful overall qualification); §13 amendment (envelope accepted as offline implementation targets; probe evidence model; arithmetic CALCULATED — PLANNER MUST RECOMPILE) PROPOSED in its pull request; runtime pagination-v2 implementation MERGED OFFLINE 2026-09-18 (the Part B code cycle: plan contract v2 with one data coordinate per group, a conditional probe only for an exactly-full page and a 2N call ceiling under 96; run-locator v2 and acquisition-record v2 carrying the completion evidence; build-manifest v2; Silver v3 with the tickers `table=stocks` group and the accepted actions event identity `sharadar-actions-event-identity/v1`; pagination admission v2; the 32 MiB whole-body ceiling, 100,000/100,000/10,000 row limits and pinned configuration targets; the accepted O-5 planner derives run 1′ = 47 coordinates / 143 writes / 94 worst-case calls and the program = 18 runs / 855 coordinates / 2,601 writes — the indicative 10 / 847 / 2,561 is superseded because the 96 ceiling binds worst-case calls) — NOT built into an image, NOT registered, NOT deployed, NEVER run; run 1 (run-20260917T221453Z-af316f8b, 2026-09-17) COMPLETED / receipt-verified / locator admitted / NOT BUILDABLE (every tickers and actions page full at 10,000 rows; 2,147 rows re-served across two tickers offsets) — historical, to be replaced whole after the qualification cycle (D-19); run-2 specification 899f2e11… SUPERSEDED / PRESERVED (no D-15, identity unconsumed); runs 2–19 ON HOLD; S10c BLOCKED; no limit or ceiling qualified
ADR-0054 historical v1 reservations + registration-historical rebinding: ACCEPTED / IN FORCE — PR #135 merged 2026-09-18T17:35:07Z (merge commit 8be462b953c33a6953429ee9edc321f843ab6ae1, approved head bbaa419dea40f53040f2d228d25fc1eb631b713c, merge tree = reviewed head tree; while PR #135 was open it was proposed and carried no authority) — evidence-only v1 reservation read (never launch authority; strict v2 parser unchanged on every execution path) and the one proven, preserved, digest-linked exception to ADR-0052 §2.3 for a runner-derived HISTORICAL binding under a moved registration; deployment impact NONE; the six R-1 cells under registration 3cf07d01… HISTORICAL / NOT PREPARED / NOT LAUNCHED; R2-BLD-CORROBORATION BLOCKED; nothing PASSED under the new registration
ADR-0055 compact build input v2:                                 ACCEPTED / IN FORCE — PR #138 merged at its exact reviewed head (merge commit recorded in the S10c evidence packet; while PR #138 was open it was proposed and carried no authority) — rows {run_identity, locator_sha256}, locator digest held before decoding, ledger-row view re-derived from the validated locator, ceiling enforced before any write, refused_input_size exit 20; 32 runs at 5,717 B inside the unchanged 8,192-B tier; v1 historical only; deployment impact BUILD + BUILD-VERIFY IMAGES, two task definitions, two registration blocks — STATED, NOT PERFORMED, NOT AUTHORIZED BY ACCEPTANCE
verification tooling corrections (PRs #115–#125):                ON MAIN — every merge at its exact reviewed head (see the register row); R-3 PASSED; registration a0155d0a… (six targets at revision 2 from release d5349d95); the six R-1 cells PASSED 2026-09-17; the one R-2 watcher attempt PATH_NOT_CREATED (Client.MissingParameter, corrected by PR #125); nothing here authorizes a launch
production image / registry digest / task-definition revision:   PUBLISHED / REGISTERED under separate written authorizations (release d5349d95; six task definitions at revision 2; registration a0155d0a…) — recorded in the owner's evidence, not here; stage a and b APPLIED; R-3 PASSED; production ingestion still NOT AUTHORIZED / NOT RUN
production ingestion/backfill/update:             NOT AUTHORIZED / NOT RUN
...
**What does not exist, and is not claimed.** No production image, no entrypoint that has ever run (two are on `main`
since PR #100, ADR-0043 accepted, refusing on any workstation before a client exists), no real exchange-calendar
...
is on `main` (ADR-0042, accepted on the PR #99 merge) — the task-side spent-identity delivery is acquisition input v2 (ADR-0044, on `main`: the owner
ledger's spent identities travel inside the input), no production compiled image configuration (ADR-0044 defines the
file, its generator and the release-attested trust chain for the image digest and revision, which an image cannot
know about itself; only synthetic ones have been generated, for the local verification images), no published image,
no R-3 tooling, no strategy logic and no cockpit change. Every adapter has only ever been exercised against synthetic fakes: **mocked
...
task entrypoints (code):                           OFFLINE / SYNTHETIC-ONLY — on main, PR #100 merged 2026-09-12T19:55:44Z (ADR-0043); closed entry selection, compiled checks, credential-environment and origin refusals before any client, receipts and exit codes; never run
delivery contracts + packaging (code):             OFFLINE / SYNTHETIC-ONLY — on main, PR #101 merged 2026-09-14T01:09:53Z (ADR-0044); release v2 attests image and configuration digests, acquisition input v2 carries spent identities, receipt line bound by digest on every outcome, Dockerfile/generator/build-context preparer prepared (source from the exact Git tree, every file held to its blob); two local verification images built once, network-disabled, never published
ADR-0038 (production run reservation):             ACCEPTED / IN FORCE — PR #96 merged 2026-09-12T15:10:12Z
...
production image:                                  NONE — two LOCAL verification images (synthetic configuration, one workstation) exist; not published, not registered
task entrypoints (code):                           ON MAIN (ADR-0043, PR #100) — offline; never run as a task, run only in local network-disabled verification containers; compiled configuration file absent from the repository by design
task-side spent-identity source:                   ACCEPTED (ADR-0044, PR #101) — inside acquisition input v2; a missing block refuses the input
...
contract unenforceable as written.** Those are two different findings, and the second is why this
was an amendment rather than a bug fix.
...
**`77a59e3f90e24f8509fa3ec36aef255150f081c6`**, merged **2026-09-07T19:53:49Z**. Its conditional
...
authored with, which is this repository's convention for a decision's own text. It **amends and
supersedes no ADR document**; **ADR-0027, ADR-0028 and ADR-0029 each remain ACCEPTED / IN FORCE**.

**Its §7 bounded implementation follow-up has since been built and independently reviewed under a
later, separate written authorization, and is carried by the open reference-enforcement pull
request** — see *The accepted owning-area navigation, and its implementation* below, which governs
the current state of both this decision's implementation and ADR-0031's.
...
referent is *we do not have it* rather than an inapplicability. **No accepted guard was relaxed to
fit this proposal's prose.**

**Accepting a decision implements nothing, and the implementation is a separate gate that was
opened separately.** On the day ADR-0030 was accepted `ref_kind` was still an open string, the
per-kind resolution sets were still unenforced, and both `downstream_refs.trade` and
`initial_planned_risk_open[].trade_ref` were still emitted as kind `source_fact` — a true record of
those days, and no longer the state of the tree. The bounded cycle that closes each of them has
since been separately authorized, built and independently reviewed, and it is **carried by the open
reference-enforcement pull request**: `ref_kind` is closed at the twenty-seven members, the per-kind
resolution sets are compiled and enforced, both trade references are re-labelled kind `trade`,
`EMBEDDED` is gated on catalogue permission **and** validated truth, `REFERENT_NOT_FOUND` lands on
the value-bearing field an ordinary read actually reaches, and all nineteen read models carry one
