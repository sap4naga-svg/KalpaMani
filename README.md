...
ADR-0046 verification tooling (code):                            OFFLINE / SYNTHETIC-ONLY / FAKES ONLY / NEVER RUN — proposed in an open, unmerged pull request (ADR-0046 PROPOSED, NOT IN FORCE); the production human-binding materializer (G-4; one flag per actor; materializes an owner declaration, verifies no identity), the R-3 verification tool (G-5; the nine accepted rows and the ten-operation cleanup on an injected client; kalpamani-r3-verification-record/v1 bound to the declared statements and the environment binding; default performs nothing) and the verification cell runner (G-6; cells enumerated from ADR-0036 §3, state derived from the ledger, reservations and records; the negative R-1 cells BLOCKED pending a withheld-release decision; R-4…R-9 enumerated, owner-run); no R-3 session, no materialization, no launch, no AWS call
