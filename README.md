...
preparation are proposed in an open pull request** (ADR-0044), and **no production image exists and no entrypoint has
ever run**.
...
ADR-0044 delivery contracts + packaging (code):   OFFLINE / SYNTHETIC-ONLY / NO IMAGE / NEVER RUN — proposed, open PR; CompiledTask v2, release v2, acquisition input v2, compiled-configuration file, receipt line on every outcome and a total validator, Dockerfile, generator and tree-exact build-context preparer; compiled configuration file absent by design
...
ADR-0043 (production task entrypoint composition): ACCEPTED / IN FORCE — PR #100 merged 2026-09-12T19:55:44Z; its §3/§4 proposals resolved by proposed ADR-0044
ADR-0044 (delivery contracts and packaging):       PROPOSED — NOT IN FORCE
production ingestion/backfill/update:             NOT AUTHORIZED / NOT RUN
...
is on `main` (ADR-0042, accepted on the PR #99 merge) — no task-side spent-identity delivery on `main` yet (ADR-0043 §3
left it an owner decision; proposed ADR-0044 carries the owner ledger's spent identities inside acquisition input v2), no
compiled image configuration (proposed ADR-0044 defines the file, its generator and the release-attested trust chain
for the image digest and revision, which an image cannot know about itself), no R-3 tooling, no strategy
...
task entrypoints (code):                           ON MAIN (ADR-0043, PR #100) — offline, never run; compiled configuration file absent by design
task-side spent-identity source:                   PROPOSED (ADR-0044, open PR) — inside acquisition input v2; a missing block refuses the input
