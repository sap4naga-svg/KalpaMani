...
ADR-0047 negative launches + permission subcells (code):         OFFLINE / SYNTHETIC-ONLY / FAKES ONLY / NEVER RUN — proposed in an open, unmerged pull request (ADR-0047 PROPOSED, NOT IN FORCE); ReleaseMode on the specification and the record, the launcher's WITHHELD and MISMATCHED steps, the negative cells passing only on the expected receipt-verified refusal; 98 permission subcells (56 executable at L3 by scripts/production_permission_cells.py under a prepared statement and a consumed per-subcell authorization, 6 evidenced by R-1, 36 BLOCKED on a task-side probe entry, the deletion role's execution path or a running task for ExecuteCommand); PR #106 correction 1 (authorization consumption, prerequisite identity, possibly committed writes and launches settled by identity, ECS task accounting, negative-evidence recovery) offline; no launch, no subcell executed, no cleanup, no AWS call, no permission granted
