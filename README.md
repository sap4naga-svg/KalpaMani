...
| **A minted capability, not a boolean** | the first revision took `binding_authorized: bool`, so any importer could pass `True` and reach the profile, identity and bucket stages with the flag never parsed. The authorization is now a capability this module mints **only after the exact flag parses** — no public constructor, no subclassing, admitted by exact type *and* mint identity, unexported, minted at one call site. A structural lookalike, a borrowed-mint lookalike, an uninitialised instance, a deep copy and a pickle round-trip are all refused; a shallow copy stays genuine, because it holds the same sentinel. **Not a claim about hostile runtime introspection** — only that no ordinary construction, subclassing, copying, deserialisation or lookalike admits |
