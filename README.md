...
ADR-0041 accepted), a **build-side pagination gate is proposed in an open pull request** (ADR-0042), and **no
...
ADR-0042 build pagination admission (code):       OFFLINE / SYNTHETIC-ONLY — proposed, open PR; one supported page shape for tickers, actions and stocks
...
ADR-0042 (build-side pagination admission):      PROPOSED — NOT IN FORCE
production ingestion/backfill/update:             NOT AUTHORIZED / NOT RUN
...
merge) that only a scripted transport has exercised — and, until ADR-0042 is accepted, no build-side refusal on
`main` of a multi-page or full-page delivery (proposed in an open pull request) — no task-side spent-identity source (an owner decision), no R-3 tooling, no strategy
...
build pagination admission (code):                 OFFLINE / SYNTHETIC-ONLY — proposed in an open PR (ADR-0042); refuses multi-page, full-first-page and inconsistent deliveries for every dataset
ADR-0038 (production run reservation):             ACCEPTED / IN FORCE — PR #96 merged 2026-09-12T15:10:12Z
