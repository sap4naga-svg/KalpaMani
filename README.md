...
offline code** — see *The production runtime foundations* below — and **no acquisition or build processing,
no production image and no entry point exists**.
...
ADR-0036 acquisition processing (code):           OFFLINE / SYNTHETIC-ONLY / NO PROVIDER ADAPTER / NO IMAGE — proposed, open PR
ADR-0036 build processing (code):                 NOT IMPLEMENTED — the build task halts after the release barrier
production ingestion/backfill/update:             NOT AUTHORIZED / NOT RUN
...
**The offline acquisition-processing path (proposed in a later, open pull request; not on `main`).** Under a
...
| **spent identities** | `identities.py`: a registry answering `UNSPENT` / `SPENT` / `UNAVAILABLE`; a ledger-backed implementation for the workstation and an always-`UNAVAILABLE` one for a task without a source — the **preliminary** check, stated as such; the **durable** guard is the conditional claim write, whose name binds the run identity and the request ordinal |
| **processing** | `processing.py`: after `RELEASED`, one credential through the accepted secrets boundary, then per request pacing → deadline-admitted fetch through an injected provider → three conditional writes (claim, payload, record) through the accepted write-only publisher over a `put_object`-only client; a payload 412 on the content-addressed name is `ALREADY_PRESENT` (ADR-0035 §3.1), a claim or record 412 is a **conflict** that halts, a definitive refusal halts, an ambiguous result halts **and** marks the state unknown; no retry, no read, no resumption |
| **locator** | published last; **COMPLETE only when every request has confirmed dispositions, no state is unknown and the document passes the accepted validator** against the run's own identity, plan and slice; PARTIAL otherwise; `NOT_PUBLISHED`, `STATE_UNKNOWN` and `NAME_OCCUPIED` reported as such and never as completion |
| **keys** | the record and claim names now carry the request ordinal (`…/acquisitions/<digest>/<run-id>.<NN>.json`, `_production_claims/<digest>/<run-id>.<NN>.json`), so identical payloads from distinct requests keep distinct provenance; locator entries carry `payload_disposition` |

**What does not exist, and is not claimed.** No build processing (no locator-driven reads into Silver, Gold or
manifests), no production image, no entry point, no provider adapter that can reach the wire — the accepted
...
acquisition processing (code):                     OFFLINE / SYNTHETIC-ONLY — proposed in an open PR; no provider adapter can reach the wire
build processing:                                  NOT IMPLEMENTED — the build task halts after the release barrier
