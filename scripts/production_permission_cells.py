...
contract. The authorized branch (``--execute-subcell <id>`` with
``--i-am-the-owner-authorizing-one-permission-subcell``) executes **one** L3 subcell:
one operation by one principal against one exact target, one transport attempt, under
that principal's own workstation profile and only after its identity is proven -- and
the cleanup branch (``--cleanup`` with ``--i-am-the-owner-authorizing-permission-cleanup``)
removes, under the control principal, every synthetic object the session's records and
attempts name and confirms each absent.
...
the attempt record written **before** the operation, naming the key it may create → the
client built **only now**, one attempt, finite timeouts → the operation → the record. An
unexpected launch is stopped at once and recorded. Every record is written exclusively
under the owner's records directory beside the launch records, where the cell runner
derives the matrix from them.
...
class _Admitted:
    ...
    __slots__ = ("binding", "environment", "inputs", "production_secret", "store", "targets")
    ...
    *,
    ...
    try:
        target = pc.resolve_target(
            cell,
            stamp=stamp,
            licensed_bucket=admitted.environment.licensed_bucket_name,
            inputs=admitted.inputs,
            targets=admitted.targets,
            production_secret=admitted.production_secret,
        )
...
def cleanup_keys(store: Any, binding: pc.PermissionBinding) -> list[tuple[pc.TargetKind, str]]:
    """Every key the session's records and attempts name under this binding, deduplicated.

    A record's created key, and an attempt's key when no record answered the attempt (the
    interrupted case: the object may or may not exist, and one delete settles it).
    """
    from production_verification_cells import permission_evidence

    evidence = permission_evidence(store, binding)
    keys: dict[str, pc.TargetKind] = {}
    for records in evidence.records.values():
        for record in records:
            if record.created_key is not None and record.binding == binding:
                keys[record.created_key] = record.target
    for subcell_id, attempts in evidence.attempts.items():
        answered = {r.started_at for r in evidence.records.get(subcell_id, ())}
        for attempt in attempts:
            if attempt.key is None or attempt.binding != binding:
                continue
            if not any(started >= attempt.started_at for started in answered):
                keys[attempt.key] = pc.subcell(subcell_id).target
    return sorted(((kind, key) for key, kind in keys.items()), key=lambda kv: kv[1])
...
def execute_cleanup(
    ...
    *,
    ...
    """The cleanup branch: the control principal removes every recorded key."""
    if running_under_automation(env, modules):
        raise PermissionToolRefusalError(
            "refused_execution_context", EXIT_REFUSED_EXECUTION_CONTEXT
        )
    ...
    keys = cleanup_keys(admitted.store, admitted.binding)
    resolved = tuple(
        (
            pc.bucket_of(
                kind,
                licensed_bucket=admitted.environment.licensed_bucket_name,
                control_bucket=admitted.targets.control_bucket_name,
            ),
            key,
        )
...
def result_lines(record: pc.PermissionRecord) -> list[str]:
    ...
    if record.started_task_id is not None:
        lines.append(
            "unexpected_task_started=yes "
            f"stop_acknowledged={'yes' if record.stop_acknowledged else 'no'}"
        )
...
def cleanup_lines(cleanup: pc.PermissionCleanup) -> list[str]:
    return [
        ...
        f"residue={len(cleanup.residue)} operations={cleanup.operations} "
    ...
    def _call(self, service: str, operation: str, **kwargs: Any) -> r3.Observation:
        ...
        if operation == "run_task":
            ...
            if failures or not tasks:
                # A 200 with a failure entry started nothing; classify it as ambiguous
                # rather than as a launch or as a denial.
                return r3.Observation(status=None, code="RunTaskFailureEntry")
            task_arn = str(tasks[0].get("taskArn", "")) or None
        return r3.Observation(
...
def main(
    ...
    *,
    ...
    try:
        ...
        return EXIT_REFUSED_ARGUMENTS
    modes = sum(
        (parsed.execute_subcell is not None, parsed.cleanup, parsed.check_record is not None)
    )
    if modes > 1:
        print(SENTENCES["refused_arguments"])
        return EXIT_REFUSED_ARGUMENTS
    ...
    if parsed.ledger is None or parsed.launch_inputs is None or parsed.records_dir is None:
        print(SENTENCES["refused_arguments"])
        return EXIT_REFUSED_ARGUMENTS
    if parsed.execute_subcell is not None and (not parsed.authorized or parsed.cleanup_authorized):
        print(SENTENCES["refused_arguments"])
        return EXIT_REFUSED_ARGUMENTS
