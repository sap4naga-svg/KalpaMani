...
captured, piped, redirected, buffered or written to a file, which is the one
thing the first attempt got wrong: the browser and device-code surface has to be
able to reach the owner. The **interactive browser/device flow completed**, the
command **exited 0**, no lingering AWS CLI process remained, and **successful
governed SSO refreshes became ONE**.
